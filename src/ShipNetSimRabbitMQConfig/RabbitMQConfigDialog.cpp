...
void RabbitMQConfigDialog::saveConfig()
{
    ...
    // Note: savePasswordToKeychain shows warning on failure
    // For now, always save password to XML as fallback
#endif

#ifndef HAVE_QTKEYCHAIN
    // No keychain available - save password to XML
    addElement("password", m_passwordEdit->text());
#else
    // Save password to XML as fallback in case keychain fails
    addElement("password", m_passwordEdit->text());
#endif
...
{
#ifdef HAVE_QTKEYCHAIN
    QKeychain::WritePasswordJob job("ShipNetSim");
