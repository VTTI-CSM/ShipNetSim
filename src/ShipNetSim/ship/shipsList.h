 ...
 */
static QVector<QMap<QString, std::any>> readShipsFile(
    QString filename,
...
loadShipsFromParameters(QVector<QMap<QString, T>> shipsDetails) {
    ...
    for (auto& parameters : shipsDetails) {
        ...
        // Convert QString values to std::any if T is QString
        if constexpr (std::is_same_v<T, QString>) {
            for (auto it = parameters.begin(); it != parameters.end(); ++it) {
                ...
                // Use FileOrderedparameters.converter to convert value
                // to appropriate type
                convertedParameters[key] =
                    param->converter(value, param->isOptional);
            }
        } else {
            // Otherwise, use the parameters directly assuming T is std::any
