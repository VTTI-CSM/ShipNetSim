 ...
 *
 * @details Holds all components required for a single network
 * simulation, including network, simulator, workers, and ships. Each
 * network in the simulation has its own APIData instance.
 */
struct SHIPNETSIM_EXPORT APIData
{
    ShipNetSimCore::OptimizedNetwork *network =
        nullptr; ///< Network instance for routing and geography
    SimulatorWorker *simulatorWorker =
        nullptr; ///< Worker for simulation processing
    ShipNetSimCore::Simulator *simulator =
        nullptr; ///< Main simulator instance
    ShipLoaderWorker *shipLoaderWorker =
        nullptr; ///< Worker for loading ship data
    QThread *workerThread =
        nullptr; ///< Dedicated thread for this network
    QMap<QString, std::shared_ptr<ShipNetSimCore::Ship>>
         ships; ///< Map of ship ID to ship instance
    bool isBusy =
        false; ///< Indicates if network is currently processing
};
...
public:
    ...
    {
        QReadLocker locker(&mDataLock);
        return mData.contains(networkName) ? mData[networkName].isBusy
                                           : false;
    }

    // Get the simulator for a network
    ShipNetSimCore::Simulator *
    getSimulator(const QString &networkName) const
    {
        QReadLocker locker(&mDataLock);
        if (!mData.contains(networkName))
        {
    ...
    getShipByID(const QString &networkName,
