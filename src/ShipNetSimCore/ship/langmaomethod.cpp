...
LangMaoMethod::getTotalResistance(const Ship &ship)
{
    units::force::newton_t wave = getWaveResistance(ship);

    units::force::newton_t wind = getWindResistance(ship);

    return (wave + wind);
}

units::force::newton_t
LangMaoMethod::getWaveResistance(const Ship &ship)
{
    units::force::newton_t ref = getWaveReflectionResistance(ship);

    units::force::newton_t motion = getWaveMotionResistance(ship);

    return ref + motion;
}

units::force::newton_t
LangMaoMethod::getWindResistance(const Ship &ship)
{
    ...
    double relativeWindSpeed = [&]() -> double {
        double heading = ship.getCurrentHeading()
    ...
    return units::force::newton_t(
        0.5 * c_d * relativeWindSpeed
        * ship.getLengthwiseProjectionArea().value());
}

units::force::newton_t
LangMaoMethod::getWaveReflectionResistance(const Ship &ship)
{
    ...
    double bf = [&]() -> double {
        double E = ship.getHalfWaterlineEntranceAngle()
                       .convert<units::angle::radians>()
                       .value();
    ...
    double alpha_T = [&]() -> double {
        ...
        double ohm =
            (ship.getSpeed().value() * w) / hydrology::G.value();
        double ke =
            k
            * std::pow(1.0f + ohm * std::cos(waveOrientationToShip),
                       2);
    ...
    double fn =
        hydrology::F_n(ship.getSpeed(), ship.getLengthInWaterline());

    double advanceCoef = [&]() {
        double C_u = std::max((double)-310.0 * bf + (double)68.0,
                              (double)10.0);
        return 1.0 + C_u * fn;
    }();

    double term1 = env.waveLength.value()
                   / ship.getLengthBetweenPerpendiculars().value();

    auto waveAmplitude = env.waveHeight.value() / 1.5f;

    return units::force::newton_t(
        (double)0.5 * water_rho.value() * hydrology::G.value()
        * std::pow(waveAmplitude, 2) * ship.getBeam().value() * bf
        * alpha_T * advanceCoef * ((double)0.19 / ship.getBlockCoef())
        * std::pow(term1, fn - (double)1.11));
}

units::force::newton_t
LangMaoMethod::getWaveMotionResistance(const Ship &ship)
{
    double fr =
        hydrology::F_n(ship.getSpeed(), ship.getLengthInWaterline());
    auto   env = ship.getCurrentEnvironment();
    double a1 =
        ((double)60.3 * std::pow(ship.getBlockCoef(), (double)1.34)
    ...
    // ITTC — Recommended Procedures and Guidelines - Seakeeping
    // Experiments Effective Date: 2021 - Revision 07 - page 4,
    // section 2.3.1 "If the longitudinal radii of gyration for pitch
    // or yaw are unknown, a value of 0.25 Lpp could be used. If the
    // transverse radius of gyration is unknown, a value between 0.35B
    // and 0.40B, depending on the ship type, could be used."
    double k_yy = 0.25; // in terms of Lpp

    double omega_delta = [&]() -> double {
        double waveAngularFreq =
            env.waveFrequency.value() * 2.0f * M_PI;
        double c1 = (double)0.4567 * (ship.getBlockCoef() / k_yy)
                    + (double)1.689;
        double term1 =
            std::sqrt(ship.getLengthBetweenPerpendiculars().value()
                      / hydrology::G.value());
        double term2 = std::pow(
            k_yy / ship.getLengthBetweenPerpendiculars().value(),
            ((double)1.0 / c1));
        ...
        if (fr < 0.05)
        {
            return ((term1 * term2 * std::pow(0.05, 0.143)
                     * waveAngularFreq)
                    / dom);
        }
        else
        {
            return ((term1 * term2 * std::pow(fr, 0.143)
                     * waveAngularFreq)
                    / dom);
        }
    }();
    ...
    double result =
        ...
        * std::exp((b1 / d1) * (1.0f - std::pow(omega_delta, d1)))
        * a1 * a2;
...
double
LangMaoMethod::getDragCoef(units::angle::degree_t angleOfAttach)
{
    (void)angleOfAttach;
