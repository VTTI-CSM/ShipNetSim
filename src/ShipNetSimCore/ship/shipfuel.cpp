...
ShipFuel::getWeight(units::volume::liter_t quantity,
                    FuelType               fuelType)
{
...
ShipFuel::convertKwhToLiters(units::energy::kilowatt_hour_t energy,
                             FuelType                       fuelType)
{
    // Convert kWh to MJ
    ...
    if (it != fuelDetails.end())
    {
        ...
        {
            std::cerr << "Calorific value not found for fuel type"
                      << std::endl;
            return units::volume::liter_t(0.0);
        }
    }
...
ShipFuel::convertLitersToKwh(units::volume::liter_t volume,
                             FuelType               fuelType)
{
    ...
    if (it != fuelDetails.end())
    {
        ...
        // Find the calorific value of the fuel type in
        // calorificValues
        auto calorificValueIt = fuelDetails.find(fuelType);
        if (calorificValueIt != fuelDetails.end())
        {
...
units::mass::kilogram_t
ShipFuel::convertLitersToCarbonDioxide(units::volume::liter_t volume,
                                       FuelType fuelType)
{
    ...
    if (it != fuelDetails.end())
    {
        ...
        auto CC = it->second.carbonContent; // Get the Carbon content
                                            // as a fraction

        // 44 is the molar mass of CO2,
        // 12 is the molar mass of carbon.
        return units::mass::kilogram_t(CC * (44.0 / 12.0) * kg);
    }
...
units::mass::kilogram_t
ShipFuel::convertLitersToSulfurDioxide(units::volume::liter_t volume,
                                       FuelType fuelType)
{
    auto kg = getWeight(volume, fuelType);
    auto it = fuelDetails.find(fuelType);
    if (it != fuelDetails.end())
    {
        ...
        auto CC = it->second.sulfureContent; // Get the Sulfur content
                                             // as a fraction

        // 64 is the molar mass of SO2,
        // 32 is the molar mass of sulfur.
        return units::mass::kilogram_t(CC * (64.0 / 32.0) * kg);
    }
