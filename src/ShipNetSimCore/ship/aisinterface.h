...
public:
    ...
    QVector<QString> getReceivedChunks(QString &shipID) const; // Retrieve received chunks for a specific ship
    void clearReceivedChunks(QString &shipID);                 // Clear received chunks for a specific ship
    QMap<QString, QVector<QString>> getAllReceivedData() const; // Retrieve all received chunks
    QMap<QString, Status> getAllStatuses() const;          // Retrieve all decoded statuses
...
private:
    ...
    units::time::second_t mTimeSinceLastTransmission;   // Time elapsed since the last transmission
    units::time::second_t mCurrentTransmissionInterval; // Current interval based on speed/state

    // Variables for receiving and decoding
    QMap<QString, QVector<QString>> mReceivedChunksMap; // Received chunks grouped by ship
    QSet<QString> mCompletedMessages;                  // Set of ships with complete messages
    QMap<QString, Status> mDecodedStatuses;            // Decoded statuses for each ship user id
    QMap<QString, units::time::second_t> mLastUpdateTime; // Time since last update for each ship

    // Constants for AIS ranges (in meters)
    static units::length::meter_t SHIP_RANGE;        // Ship-to-Ship range
    static units::time::second_t MAX_INACTIVE_TIME;  // Max time before removing a ship


    // Helper methods
    QString encodeBinary() const;    // Helper to encode ship data into binary string
    void decodeBinary(const QString& binaryData,
                      const Ship *transmittingShip); // Decode binary for a specific ship
    ...
    static units::length::meter_t calculateDistance(const Ship *ship1,
                                                    const Ship *ship2);
};
