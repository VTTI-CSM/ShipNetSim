...
// static constexpr units::density::kilograms_per_cubic_meter_t
// WATER_RHO =
                 ...
                 units::temperature::celsius_t temperature)
{
    ...
    double T =
        std::isnan(temperature.value()) ? 15.0 : temperature.value();
    ...
    double rhoS = rho0 + (b0 * S) + (b1 * std::pow(S, 1.5))
                  ...
                  + (c1 * std::pow(T, 2) * S)
                  + (c2 * std::pow(T, 3) * S) + (d0 * std::pow(S, 2));

    return units::density::kilograms_per_cubic_meter_t(rhoS);
}

inline units::velocity::meters_per_second_t
get_nue(double salinity, units::temperature::celsius_t temp)
{

    salinity = std::isnan(salinity) ? 35.0 : salinity;
    temp     = std::isnan(temp.value())
                   ? units::temperature::celsius_t(15.0)
                   : temp;
    ...
    if (temp.value() < 0.0)
    {
        ...
        // qCritical() << "temperature must be greater than 0
        // celcuis!"; return units::velocity::meters_per_second_t(0);
    }
                  ...
                  units::length::meter_t               ship_length)
{
    ...
    return ship_speed.value() / sqrt(ship_length.value() * G.value());
}
