...
units::length::meter_t AISInterface::SHIP_RANGE =
    units::length::meter_t(37040.0); // 20 nautical miles
units::time::second_t AISInterface::MAX_INACTIVE_TIME =
    units::time::second_t(60); // 60 seconds

AISInterface::AISInterface()
    : mHost(nullptr)
    , nextChunkIndex(0)
    , mTimeSinceLastTransmission(0)
    , mCurrentTransmissionInterval(10)
{
}
...
void AISInterface::step(
    std::function<void(const QString &)> sendFunction,
    units::time::second_t                timeStep)
{
    mTimeSinceLastTransmission += timeStep;

    // Check for inactive ships and remove them
    for (auto it = mLastUpdateTime.begin();
         it != mLastUpdateTime.end();)
    {
        it.value() += timeStep;
        if (it.value() > MAX_INACTIVE_TIME)
        {
            ...
            it = mLastUpdateTime.erase(
                it); // Remove the ship from the map
        }
        else
        {
            ++it;
    ...
    if (mTimeSinceLastTransmission >= mCurrentTransmissionInterval)
    {
        ...
        mTimeSinceLastTransmission = units::time::second_t(
            0); // Reset time since last transmission
    }
}
                                  ...
                                  const Ship    *transmittingShip)
{
    ...
    if (distance <= SHIP_RANGE)
    {
        ...
        mReceivedChunksMap[transmittingShip->getUserID()].append(
            chunk);
...
QVector<QString>
AISInterface::getReceivedChunks(QString &shipID) const
{
...
AISInterface::getAllReceivedData() const
{
...
QString AISInterface::encodeBinary() const
{
    ...
    int latBits =
        static_cast<int>(pos.getLatitude().value() * 600000);
    binaryData += toBinary(latBits, 27);

    // Encode Longitude (28 bits, scaled by 600,000)
    int lonBits =
        static_cast<int>(pos.getLongitude().value() * 600000);
    ...
    // TODO: The course is equal to heading now since we do not have
    // side waves
    int cogBits =
        static_cast<int>(mHost->getCurrentHeading().value() * 10);
    binaryData += toBinary(cogBits, 12);

    // Encode True Heading (9 bits)
    int headingBits =
        static_cast<int>(mHost->getCurrentHeading().value());
    binaryData += toBinary(headingBits, 9);

    // Encode Navigation Status (4 bits)
    binaryData +=
        toBinary(static_cast<int>(mHost->getNavigationStatus()), 4);
    ...
    for (size_t i = 0; i < 20; ++i)
    {
        ...
        {
            binaryData +=
                toBinary(static_cast<int>(shipName[i].unicode()), 6);
        }
        else
        {
                                ...
                                const Ship    *transmittingShip)
{
    ...
    units::angle::degree_t latitude = units::angle::degree_t(
        fromBinary(binaryData.mid(pos, 27)) / 600000.0);
    pos += 27;

    // Decode Longitude (28 bits, scaled by 600,000)
    units::angle::degree_t longitude = units::angle::degree_t(
        fromBinary(binaryData.mid(pos, 28)) / 600000.0);
    pos += 28;

    // Decode Speed Over Ground (10 bits, scaled by 10)
    status.speed = units::velocity::knot_t(
        fromBinary(binaryData.mid(pos, 10)) / 10.0);
    pos += 10;

    // Decode Course Over Ground (12 bits, scaled by 10)
    status.course = units::angle::degree_t(
        fromBinary(binaryData.mid(pos, 12)) / 10.0);
    pos += 12;

    // Decode True Heading (9 bits)
    status.heading =
        units::angle::degree_t(fromBinary(binaryData.mid(pos, 9)));
    pos += 9;

    // Decode Navigation Status (4 bits)
    status.navStatus = static_cast<Ship::NavigationStatus>(
        fromBinary(binaryData.mid(pos, 4)));
    pos += 4;
