...
static const QMap<Ship::ShipAppendage, double> &get_k_2_iMap()
{
    static QMap<Ship::ShipAppendage, double> k_2_iMap = {
        ...
        {Ship::ShipAppendage::exposed_shafts_10_degree_with_buttocks,
         (double)1.0},
        {Ship::ShipAppendage::exposed_shafts_20_degree_with_buttocks,
         (double)4.0},
...
double HoltropMethod::calc_c_7(const Ship &ship)
{
    if ((ship.getBeam() / ship.getLengthInWaterline()) < 0.11)
    {
        return (double)0.229577
               * pow((ship.getBeam() / ship.getLengthInWaterline()),
                     (double)0.33333);
    }
    else if ((ship.getBeam() / ship.getLengthInWaterline()).value()
             < (double)0.25)
    {
    ...
    {
        return ((double)0.5
                - ((double)0.0625 * (ship.getLengthInWaterline())
                   / ship.getBeam()));
    }
}
...
{
    return (
        (double)2223105.0 * pow(get_C7(ship), (double)3.78613)
        ...
        * pow(ship.getBulbousBowTransverseArea().value(), (double)1.5)
        / (ship.getBeam().value() * ship.getMeanDraft().value()
           * ((double)0.31
              ...
              - ship.getBulbousBowTransverseAreaCenterHeight()
                    .value())));
}
...
double HoltropMethod::calc_c_16(const Ship &ship)
{
    ...
    {
        return ((double)8.07981 * ship.getPrismaticCoef()
                - (double)13.8673
                      * pow(ship.getPrismaticCoef(), (double)2.0)
                + (double)6.984388
                      * pow(ship.getPrismaticCoef(), (double)3.0));
    }
    else
    {
        return (double)1.73014
               - (double)0.7067 * ship.getPrismaticCoef();
    }
}
...
double HoltropMethod::calc_m_1(const Ship &ship)
{
    return (
        (double)0.0140407
            * (ship.getLengthInWaterline() / ship.getMeanDraft())
                  .value()
        - (double)1.75254
              * pow(ship.getVolumetricDisplacement().value(),
                    ((double)1.0 / (double)3.0))
    ...
    units::velocity::meters_per_second_t customSpeed)
{
    ...
    return (get_C15(ship) * (double)0.4
...
double HoltropMethod::calc_m_3(const Ship &ship)
{
    return (
        (double)-7.2035
        * pow((ship.getBeam() / ship.getLengthInWaterline()).value(),
              (double)0.326869)
        * pow((ship.getMeanDraft() / ship.getBeam()).value(),
              (double)0.605375));
}

double HoltropMethod::calc_c_17(const Ship &ship)
{
    return ((double)6919.3
            * pow(ship.getMidshipSectionCoef(), (double)-1.3346)
            * pow((ship.getVolumetricDisplacement().value()
                   / pow(ship.getLengthInWaterline().value(),
                         (double)3.0)),
                  (double)2.00977)
            * pow((ship.getLengthInWaterline() / ship.getBeam())
                      - (double)2.0,
                  (double)1.40692));
}
    ...
    units::velocity::meters_per_second_t customSpeed)
{
    ...
    return units::force::newton_t(
        get_C1(ship) * get_C2(ship) * get_C5(ship) * water_rho
        * hydrology::G * ship.getVolumetricDisplacement()
        * exp(((get_m1(ship) * pow(fn, d))
               + (calc_m_4(ship, customSpeed)
                  * cos(get_lambda(ship) * pow(fn, (double)-2.0))))));
}
    ...
    units::velocity::meters_per_second_t customSpeed)
{
    ...
    return units::force::newton_t(
        get_C1(ship) * get_C2(ship) * get_C5(ship) * water_rho
        * hydrology::G * ship.getVolumetricDisplacement()
        * exp(get_m3(ship) * pow(fn, d)
              + calc_m_4(ship)
                    * cos(get_lambda(ship) * pow(fn, -2.0))));
}
    ...
    units::velocity::meters_per_second_t customSpeed)
{
    ...
    if (std::isnan(customSpeed.value()))
    {
        Rn = hydrology::R_n(ship.getSpeed(),
                            ship.getLengthInWaterline(), env.salinity,
                            env.temperature);
    }
    else
    {
        Rn = hydrology::R_n(customSpeed, ship.getLengthInWaterline(),
                            env.salinity, env.temperature);
    }
    return (double)0.075
           / pow((log10(Rn) - (double)2.0), (double)2.0);
}
...
{
    return (double)1.0
           + (double)0.011 * (double)getCStenByAftShape(ship);
}
...
double HoltropMethod::calc_k_1(const Ship &ship)
{
    ...
    // double waterDepth =
    // ship.getCurrentEnvironment().waterDepth.value();
    ...
    return (
        ...
        + (double)0.487118 * get_C14(ship)
              ...
              * pow(ship.getMeanDraft().value()
                    ...
                    (double)0.46106)
              * pow(ship.getLengthInWaterline().value()
                        / ship.getRunLength().value(),
                    (double)0.121563)
              * pow(pow(ship.getLengthInWaterline().value(),
                        (double)3.0)
                        / ship.getVolumetricDisplacement().value(),
                    (double)0.36486)
...
{
    if (ship.getDraftAtForward() / ship.getLengthInWaterline()
        <= 0.04)
    {
...
double HoltropMethod::calc_delta_c_A(const Ship &ship)
{
    ...
    else
    {
        auto sR = ship.getSurfaceRoughness()
                      ...
                      .value(); // It has to be in m
        return ((double)0.105 * pow(sR, ((double)1.0 / (double)3.0))
                - (double)0.005579)
               / pow(ship.getLengthInWaterline().value(),
                     ((double)1.0 / (double)3.0));
    }
...
double HoltropMethod::calc_c_A(const Ship &ship)
{
    return (
        (double)0.00546
            * pow(ship.getLengthInWaterline().value() + (double)100.0,
                  (double)-0.16)
        - (double)0.002
        + (double)0.003
              * sqrt(
                  (ship.getLengthInWaterline().value() / (double)7.5))
              * pow(ship.getBlockCoef(), (double)4.0) * get_C2(ship)
    ...
    units::velocity::meters_per_second_t customSpeed)
{
    ...
    return (s.value()
            / sqrt((double)2.0 * hydrology::G.value()
                   * (ship.getDraftAtForward().value()
                      / (ship.getBeam().value()
                         + (ship.getBeam().value()
    ...
    units::velocity::meters_per_second_t customSpeed)
{
    ...
    if (std::isnan(customSpeed.value()))
    {
        fr = hydrology::F_n(ship.getSpeed(),
                            ship.getLengthInWaterline());
    }
    else
    {
    ...
    units::length::meter_t hf =
        ship.getPrismaticCoef() * ship.getMidshipSectionCoef()
        * ((ship.getBeam() * ship.getMeanDraft())
           / (ship.getLengthInWaterline()))
        * ((double)136.0 - (double)316.3 * fr) * pow(fr, (double)3.0);
...
{
    if ((ship.getBeam() / ship.getDraftAtAft()).value()
        <= (double)5.0)
    {
        return ((ship.getWettedHullSurface()
                ...
                * (ship.getBeam() / ship.getDraftAtAft()))
            .value();
    }
    else
    {
        return (
             ...
             * ((double)7.0 * (ship.getBeam() / ship.getDraftAtAft())
                - (double)25.0))
            / (ship.getLengthInWaterline()
               * ship.getPropellers()->front()->getPropellerDiameter()
               * ((ship.getBeam() / ship.getDraftAtAft())
                  - (double)3.0)));
    }
}
...
double HoltropMethod::calc_c_19(const Ship &ship)
{
    ...
    {
        return (((double)0.12997)
                    / ((double)0.95 - ship.getBlockCoef())
                - ((double)0.11056)
                      / (((double)0.95 - ship.getPrismaticCoef())));
    }
    else
    {
        return ((((double)0.18567)
                ...
                - (double)0.71276
                + (double)0.38648 * ship.getPrismaticCoef());
    }
}
...
{
    return ((double)1.45 * ship.getPrismaticCoef() - (double)0.315
    ...
    units::velocity::meters_per_second_t customSpeed)
{
    ...
    return (
        ...
        / ((double)0.5 * water_rho.value() * pow(s, (double)2.0)
    ...
    units::velocity::meters_per_second_t customSpeed)
{
    ...
    else if (ship.getPropellers()->size() == 2)
    {
        ...
        return ((double)0.3095 * ship.getBlockCoef()
                + (double)10.0 * CV * ship.getBlockCoef()
                - (double)0.23
                      * (ship.getPropellers()
                             ->front()
                             ->getPropellerDiameter()
...
double HoltropMethod::calc_t(const Ship &ship)
{
    if (ship.getPropellers()->size() == 1)
    {
        return ((((double)0.25014
                 ...
                 / pow(((double)1.0 - ship.getPrismaticCoef()
                        + (double)0.0225
                              * ship.getLongitudinalBuoyancyCenter()),
                       (double)0.01762))
                + (double)0.0015
                      * get_C_SternMap().value(
                          ship.getSternShapeParam()));
    }
...
double get_k_2_i(const Ship &ship, Ship::ShipAppendage appendage)
{
    (void)ship;
...
double
HoltropMethod::calc_equivalentAppendageFormFactor(const Ship &ship)
{
    ...
    for (auto it = appsMap.constBegin(); it != appsMap.constEnd();
         ++it)
    {
        const Ship::ShipAppendage &appendage = it.key();
        double                     value     = it.value().value();
        k_2_eq +=
            (((double)1.0 + get_k_2_i(ship, appendage)) * value);
    }
...
units::force::newton_t HoltropMethod::getfrictionalResistance(
    const Ship                          &ship,
    units::velocity::meters_per_second_t customSpeed)
{
    units::velocity::meters_per_second_t s;
    ...
    return units::force::newton_t(
        calc_C_F(ship, s) * (double)0.5 * water_rho.value()
        * pow(s.value(), (double)2.0)
        * ship.getWettedHullSurface().value() * calc_k_1(ship));
}

units::force::newton_t HoltropMethod::getAppendageResistance(
    const Ship                          &ship,
    units::velocity::meters_per_second_t customSpeed)
{
    units::velocity::meters_per_second_t s;
    ...
    return units::force::newton_t(
        (double)0.5 * water_rho.value() * pow(s.value(), (double)2.0)
    ...
    units::velocity::meters_per_second_t customSpeed)
{
    ...
    return units::force::newton_t(
        ...
        * (ship.getWettedHullSurface()
           + ship.getTotalAppendagesWettedSurfaces())
              .value());
}

units::force::newton_t HoltropMethod::getWaveResistance(
    const Ship                          &ship,
    units::velocity::meters_per_second_t customSpeed)
{
    units::velocity::meters_per_second_t s;
    ...
    double fn = hydrology::F_n(s, ship.getLengthInWaterline());
    ...
    else
    {
        ...
        return Rwa
               + (((double)20 * fn - (double)8) / (double)3.0)
                     * (Rwb - Rwa);
    }
}

units::force::newton_t HoltropMethod::getBulbousBowResistance(
    const Ship                          &ship,
    units::velocity::meters_per_second_t customSpeed)
{
    units::velocity::meters_per_second_t s;
    ...
    auto result = units::force::newton_t(
        (double)0.11 * water_rho.value() * hydrology::G.value()
        * pow(sqrt(ship.getBulbousBowTransverseArea().value()),
              (double)3.0)
        * (pow(fri, (double)3.0)
           / ((double)1.0 + pow(fri, (double)2.0)))
        * exp((double)-3.0 * pow(get_PB(ship, s), (double)-2.0)));
    return (std::isnan(result.value())) ? units::force::newton_t(0.0)
                                        : result;
}
...
units::force::newton_t HoltropMethod::getAirResistance(
    const Ship                          &ship,
    units::velocity::meters_per_second_t customSpeed)
{
    double s = 0.0L;
    ...
    return units::force::newton_t(
        (double)0.5 * hydrology::AIR_RHO.value()
        * hydrology::AIR_DRAG_COEF
        * ship.getLengthwiseProjectionArea().value()
        * pow(s, (double)2.0));
}

units::force::newton_t HoltropMethod::getTotalResistance(
    const Ship                          &ship,
    units::velocity::meters_per_second_t customSpeed)
{
    ...
    units::force::newton_t air = getAirResistance(ship, customSpeed);

    return friction + appendage + correlation + wave + bulbous; // +
    // air;
}
    ...
    units::velocity::meters_per_second_t customSpeed)
{
    ...
    double den = 0.5 * water_rho.value()
                 * ship.getWettedHullSurface().value() * s.value()
                 * s.value();
...
HoltropMethod::calc_SpeedOfAdvance(
    const Ship                          &ship,
    units::velocity::meters_per_second_t customSpeed)
{
    units::velocity::meters_per_second_t s;
