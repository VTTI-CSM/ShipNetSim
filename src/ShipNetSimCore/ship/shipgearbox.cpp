...
void ShipGearBox::initialize(
    Ship *host, QVector<IShipEngine *> engines,
...
void ShipGearBox::setParameters(
    const QMap<QString, std::any> &parameters)
{
    mGearRationTo1 = Utils::getValueFromMap<double>(
        parameters, "GearboxRatio", -1.0);
    if (mGearRationTo1 < 0)
    {
    ...
    mEfficiency = Utils::getValueFromMap<double>(
        parameters, "gearboxEfficiency", -1.0);
    if (mEfficiency < 0)
    {
        mEfficiency = 1.0;
...
ShipGearBox::getOutputRPM() const
{
    ...
    // if multiple engines are connected, returned weighted average
    // rpm
    double totalPower =
        0; // To store the sum of the power of all engines.
...
ShipGearBox::getOutputRPMRange() const
{
    ...
    // Divide by gear ratio to get the output RPM.
    return {outputLowRPM / mGearRationTo1,
            outputHighRPM / mGearRationTo1};
}

units::power::kilowatt_t ShipGearBox::getOutputPower()
{
    units::power::kilowatt_t totalPower =
...
void ShipGearBox::setEngineTargetState(
    IShipEngine::EngineProperties newState)
{
               ...
               == IShipEngine::EngineOperationalLoad::UserDefined)
    {
        ...
        // if multiple engines are connected, returned weighted
        // average rpm
        double totalPower =
            0; // To store the sum of the power of all engines.
        ...
        // Return 0 if the total power is zero to avoid division by
        // zero.
        if (totalPower == 0)
        {
            return;
    ...
    units::angular_velocity::revolutions_per_minute_t rpm)
{
    ...
    for (const auto &engine : mEngines)
    {
        ...
        totalWeightedEfficiency +=
            engineProperties.efficiency
            * engineProperties.breakPower.value();
    }

    // Combine the properties
    combinedProperties.breakPower = totalBreakPower;

    // Calculate the overall efficiency as the weighted average based
    // on power
    ...
    {
        combinedProperties.efficiency =
            0.0; // Set efficiency to zero if no power
    }
    ...
    units::angular_velocity::revolutions_per_minute_t rpm)
{
    ...
    combinedProperties.RPM =
        rpm; // This is the output RPM after the gearbox's reduction
