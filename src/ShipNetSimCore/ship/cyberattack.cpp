...
CyberAttack::CyberAttack(AttackType type, QObject* parent)
    : QObject(parent), mAttackType(type), mAttackActive(false), mTarget(nullptr) {}
...
void CyberAttack::executeAttack() {
    ...
    if (mAttackType == GPS_Spoofing) {
        ...
        units::angle::degree_t spoofedLatitude = position.getLatitude();  // Initialize with real position
        units::angle::degree_t spoofedLongitude = position.getLongitude(); // Initialize with real position

        // Apply an offset to simulate the spoofing only if under attack
        spoofedLatitude = position.getLatitude() +
                          units::angle::degree_t((rand() % 10 - 5) * 0.001);  // Random shift by up to 0.005 degrees
        spoofedLongitude = position.getLongitude() +
