...
void ShipEngine::initialize(Ship                    *host,
                            QVector<IEnergySource *> energySources,
                            const QMap<QString, std::any> &parameters)
{
    ...
    connect(this, &ShipEngine::operationalLoadChanged, this,
            &ShipEngine::handleOperationalDetailsChange,
            Qt::QueuedConnection);

    connect(this, &ShipEngine::engineTargetStateChanged, this,
            &ShipEngine::handleTargetStateChange,
            Qt::QueuedConnection);

    connect(this, &ShipEngine::engineOperationalTierChanged, this,
            &ShipEngine::handleOperationalDetailsChange,
            Qt::QueuedConnection);
...
void ShipEngine::handleOperationalDetailsChange()
{
             ...
             == EngineOperationalLoad::UserDefined)
    {
        if (getCurrentOperationalTier()
            == IShipEngine::EngineOperationalTier::tierII)
        {
            setEngineTargetState(
                mUserEngineCurveInDefaultTier.back());
        }
        else
        {
            setEngineTargetState(
                mUserEngineCurveInNOxReducedTier.back());
        }
    }
    else
    {
...
void ShipEngine::setParameters(
    const QMap<QString, std::any> &parameters)
{
    mId = Utils::getValueFromMap<unsigned int>(parameters, "EngineID",
                                               counter);
    ...
    {
        std::sort(
            mEngineNOxReducedTierPropertiesPoints.begin(),
            mEngineNOxReducedTierPropertiesPoints.end(),
            [](const EngineProperties &a, const EngineProperties &b) {
                return EngineProperties::compareByBreakPower(a, b,
                                                             true);
            });
    }
    ...
    if (!mUserEngineCurveInDefaultTier.empty())
    {
        ...
        std::sort(
            mUserEngineCurveInDefaultTier.begin(),
             ...
             && !mUserEngineCurveInNOxReducedTier.empty())
    {
        ...
        std::sort(
            mUserEngineCurveInNOxReducedTier.begin(),
...
bool ShipEngine::selectCurrentEnergySourceByFuelType(
...
{
    // the brake power should be increased to consider the losses due
    // to the efficiency of the engine. this value should be
    // calculated by the SOF as reported by the manufacturer
...
void ShipEngine::updateEngineOperationalState()
{
    ...
    double lambda =
        getHyperbolicThrottleCoef(mHost->getSpeed(), expHgRes);
    ...
    units::power::kilowatt_t mRawPower =
        lambda * getEngineTargetState().breakPower;
    ...
    auto r = getEnginePropertiesAtPower(mRawPower,
                                        getCurrentOperationalTier());
...
units::torque::newton_meter_t ShipEngine::getBrakeTorque()
{
    units::torque::newton_meter_t result =
        units::torque::newton_meter_t(
            (getEngineCurrentState()
                 .breakPower.convert<units::power::watt>()
                 .value())
            / getEngineCurrentState()
                  .RPM
                  .convert<
    ...
    bool                                 isExperiencingHighResistance)
{
    ...
    double lambda =
        (double)1.0 / (1.0 + exp(-7.82605 * (dv - 0.42606)));
...
ShipEngine::estimateEnginePowerCurve()
{

    double P_M = getEngineTargetState().breakPower.value();
    double omega_M =
        getEngineTargetState()
            .RPM
    ...
    std::vector<double>       omegas =
        Utils::linspace_step(0.0, omega_M, 10);
    ...
    for (const auto &omega : omegas)
    {
        ...
        /// In International Marine and Offshore Engineering
        /// Conference, Portugal.
        ep.breakPower = units::power::kilowatt_t(
            P1 * omega + P2 * std::pow(omega, 2)
            + P3 * std::pow(omega, 3));
