...
QThreadStorage<QVector<std::shared_ptr<SeaPort>>>
    SeaPortLoader::mPorts;
...
bool SeaPortLoader::loadPortsFromFile(const QString &filePath)
{
    ...
    {
        qWarning() << "Error loading sea ports from file: "
                   << filePath;
    }
...
std::shared_ptr<SeaPort> SeaPortLoader::getClosestPortToPoint(
    ...
    units::length::meter_t        maxDistance)
{
    ...
    {
        // If port has a water point, use that for distance
        // calculation
...
SeaPortLoader::readSeaPorts(const char *filename)
{
    ...
    GDALDataset *dataset = static_cast<GDALDataset *>(GDALOpenEx(
        filename, GDAL_OF_VECTOR, nullptr, nullptr, nullptr));
    if (dataset == nullptr)
    {
    ...
    while ((feature = layer->GetNextFeature()) != nullptr)
    {
        ...
        {
            const char *country =
                feature->GetFieldAsString("Country");
            const char *nameWoDiac =
                feature->GetFieldAsString("NameWoDiac");
                       ...
                       == wkbPoint)
            {
                ...
                try
                {
                    ...
                    {
                        statusText =
                            "Approved by national standardisation "
                            "body";
                    }
                    else if (qStatus == "RL")
                    {
                        statusText =
                            "Recognised location - Existence and "
                    ...
                    {
                        statusText =
                            "Request from credible national sources "
                    ...
                    {
                        statusText =
                            "Original entry not verified since date "
                            "indicated";
                    }
                    else if (qStatus == "XX")
                    {
                        statusText =
                            "Entry that will be removed from the "
...
SeaPortLoader::loadFirstAvailableSeaPorts()
{
    ...
    QString filePath = Utils::getFirstExistingPathFromList(
        seaPortsLocs, QVector<QString>("geojson"));
...
{
    QString filePath = Utils::getFirstExistingPathFromList(
        locations, QVector<QString>("geojson"));
