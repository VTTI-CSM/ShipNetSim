...
// Default constructor, initializes members with NaN and default
// values.
Point::Point()
    : mOGRPoint(std::nan("No Value"), std::nan("No Value"))
    , // Initialize NaN.
    mUserID("")
    , // Initialize userID with an empty string.
    mIsPort(false)
    , // Initialize mIsPort with false.
    mDwellTime(
        units::time::second_t(0.0)) // Initialize mDwellTime with 0.
{
}
...
Point::Point(units::length::meter_t xCoord,
    ...
    : mOGRPoint(xCoord.value(), yCoord.value())
    , // Initialize the Coord.
    mUserID(ID)
    , // Initialize mIsPort with false.
    mIsPort(false)
    , // Initialize userID with ID.
    mDwellTime(
        units::time::second_t(0.0)) // Initialize index with index.
{
...
OGRPoint Point::getGDALPoint() const
{
    return mOGRPoint;
}
...
void Point::transformDatumTo(OGRSpatialReference *targetSR)
{
    ...
    {
        throw std::runtime_error(
            "Target spatial reference is not projected!");
    }
}
...
GPoint Point::reprojectTo(OGRSpatialReference *targetSR)
{
    ...
    return GPoint(units::angle::degree_t(x),
                  units::angle::degree_t(y), mUserID, *targetSR);
}
...
Point::Point(units::length::meter_t xCoord,
    ...
    : mOGRPoint(xCoord.value(), yCoord.value())
    , // Initialize the Coord.
    mUserID("temporary point")
    , // Initialize mIsPort with false.
    mIsPort(false)
    , // Initialize userID with a default value.
    // Initialize index with the minimum possible value for unsigned
    // int.
    mDwellTime(units::time::second_t(0.0))
{
    OGRSpatialReference *SR =
        getDefaultProjectionReference()->Clone();
...
// Function to check if the point is valid (i.e., coordinates are not
// NaN).
bool Point::isValid()
{
    // Return true if neither mx nor my is NaN.
    return !(std::isnan(mOGRPoint.getX())
             || std::isnan(mOGRPoint.getY()));
}

// Function to calculate the distance between the current point and
// endPoint.
units::length::meter_t
Point::distance(const Point           &endPoint,
...
QString Point::toString(const QString &format,
                        int            decimalPercision) const
{
    QString str;
    QString xStr =
        QString::number(mOGRPoint.getX(), 'f',
                        decimalPercision); // Convert x-coordinate to
                                           // string in decimal format
    QString yStr =
        QString::number(mOGRPoint.getY(), 'f',
                        decimalPercision); // Convert y-coordinate to
                                           // string in decimal format
...
Point Point::pointAtDistanceAndHeading(
    ...
    units::angle::degree_t heading) const
{
    ...
    double newX =
        x().value() + distance.value() * cos(headingRadians);
    double newY =
        y().value() + distance.value() * sin(headingRadians);
    ...
    return Point(units::length::meter_t(newX),
                 units::length::meter_t(newY), "NewPoint",
                 *currentSR);
}
...
Point Point::getMiddlePoint(const Point &endPoint)
{
    ...
    return Point(units::length::meter_t(midX),
                 units::length::meter_t(midY));
}
...
std::size_t
Point::Hash::operator()(const std::shared_ptr<Point> &p) const
{
    if (!p)
...
std::size_t std::hash<ShipNetSimCore::Point>::operator()(
    const ShipNetSimCore::Point &p) const
{
