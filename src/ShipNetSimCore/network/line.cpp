...
bool Line::intersects(const Line &other, bool ignoreEdgePoints) const
{
    ...
    // Special cases
    if (o1 == Line::Collinear && onSegment(p1, p2, q1))
        return true; // p2 is on the segment p1-q1
    if (o2 == Line::Collinear && onSegment(p1, q2, q1))
        return true; // q2 is on the segment p1-q1
    if (o3 == Line::Collinear && onSegment(p2, p1, q2))
        return true; // p1 is on the segment p2-q2
    if (o4 == Line::Collinear && onSegment(p2, q1, q2))
        return true; // q1 is on the segment p2-q2
...
Point Line::getPointByDistance(
    units::length::meter_t
        distance, // Distance from the 'from' point.
    std::shared_ptr<Point> from)
    const // Starting point for measurement.
{
...
// the line.
units::length::meter_t
Line::getPerpendicularDistance(const Point &point)
...
Point Line::getNearestPoint(const std::shared_ptr<Point> &point) const
{
...
units::length::meter_t
Line::distanceToPoint(const std::shared_ptr<Point> &point) const
{
    ...
    const std::shared_ptr<Point> &point) const // Point to be checked.
{
    ...
    // Calculate the x and y coordinates of the line's direction
    // vector.
    ...
    auto result =
        dirX.value() * relY.value() - dirY.value() * relX.value();
...
AlgebraicVector
Line::toAlgebraicVector(const std::shared_ptr<Point> startPoint) const
{
    Point begin,
...
// {
//     return startPoint()->isPointInsideBoundingBox(min_point,
//     max_point) ||
//            endPoint()->isPointInsideBoundingBox(min_point,
//            max_point);
// }
...
QString Line::toString(const QString &format,
