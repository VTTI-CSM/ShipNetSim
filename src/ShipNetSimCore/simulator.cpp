...
Simulator::Simulator(OptimizedNetwork              *network,
                     ...
                     bool isExternallyControlled, QObject *parent)
    : QObject{parent}
    , mShips(shipList)
...
{
    qDebug() << "Simulator initialized with" << mShips.size()
             << "ships.";
    ...
    // Define default names for trajectory and summary files with
    // current time
    mTrajectoryFilename = DefaultInstantaneousTrajectoryFilename
                          + QString::number(simulation_serial_number)
                          + ".csv";
...
Simulator::~Simulator()
{
    qDebug() << "Simulator destructor called. Cleaning up network "
                "and ships.";
...
{
    qDebug()
        << "Moving Simulator and associated objects to new thread.";
...
{
    qDebug() << "Starting studyShipsResistance for " << mShips.size()
             << " ships.";

    QString trajectoryFullPath =
        (mExportTrajectory)
            ? QDir(mOutputLocation).filePath(mTrajectoryFilename)
            : "";
    ...
    for (auto &ship : mShips)
    {
        qDebug() << "Calculating resistance for Ship ID:"
                 << ship->getUserID();

        // double s[] =
        // {15.0, 15.5, 16.0, 16.5, 17.0, 17.5, 18.0, 18.5, 19.0};

        for (auto speedStep : Utils::linspace_step(
                 0, ship->getMaxSpeed()
                        .convert<units::velocity::knot>()
                        .value()))
        {
            exportLine.clear(); // Ensure exportLine is cleared at the
                                // start of each speed
            ...
            {
                QString errorMsg =
                    QString("Ship ID: %1 - Missing calm "
                            "resistance strategy.")
                        .arg(ship->getUserID());
            ...
            auto speed =
                ss; // units::velocity::knot_t(speedStep).
                    // convert<units::velocity::meters_per_second>();
            auto fraudNo =
                hydrology::F_n(speed, ship->getLengthInWaterline());

            auto airRes =
                ship->getCalmResistanceStrategy()->getAirResistance(
                    *ship);
            auto bulbousRes = ship->getCalmResistanceStrategy()
                                  ->getBulbousBowResistance(*ship);
            auto transomRes =
                ship->getCalmResistanceStrategy()
                    ->getImmersedTransomPressureResistance(*ship);
            auto appendageRes = ship->getCalmResistanceStrategy()
                                    ->getAppendageResistance(*ship);
            auto waveRes =
                ship->getCalmResistanceStrategy()->getWaveResistance(
                    *ship);
            auto frictionalRes = ship->getCalmResistanceStrategy()
                                     ->getfrictionalResistance(*ship);
            auto modelCorRes =
                ship->getCalmResistanceStrategy()
                    ->getModelShipCorrelationResistance(*ship);
            auto totalRes = ship->calculateTotalResistance();
            ...
            auto FRI =
                ship->getCalmResistanceStrategy()->calc_F_n_i(*ship);
            ...
            stream1
                ...
                << ","
                << bulbousRes.convert<units::force::kilonewton>()
                       .value()
                << ","
                << transomRes.convert<units::force::kilonewton>()
                       .value()
                << ","
                << appendageRes.convert<units::force::kilonewton>()
                       .value()
                << ","
                << waveRes.convert<units::force::kilonewton>().value()
                << ","
                << frictionalRes.convert<units::force::kilonewton>()
                       .value()
                << ","
                << modelCorRes.convert<units::force::kilonewton>()
                       .value()
                << ","
                << totalRes.convert<units::force::kilonewton>()
                       .value()
                << "," << totalResCoef;
    ...
    for (auto &ship : mShips)
    {
        ...
        for (auto &speedStep : Utils::linspace_step(
                 0.0,
                 maxSpeed.convert<units::velocity::knot>().value(),
                 1.0))
        {
            ...
            // Step 4: Write propeller shaft power at the calculated
            // RPM
            ...
            for (auto &engine : propeller->getDrivingEngines())
            {
                ...
                {
                    stream2 << QString::number(
                        engine->getEnginePropertiesAtRPM(rpm)
                            .breakPower.value(),
                        'f', 2)
                            << ",";
                ...
                { // if it is outside the range, do not export it
                    continue;
                }
            }
...
void Simulator::addShipToSimulation(std::shared_ptr<Ship> ship)
{
    ...
    // Call resumeSimulation outside of the locked section to prevent
    // deadlock
    if (mIsSimulatorPaused)
        this->resumeSimulation(false);
}

void Simulator::addShipsToSimulation(
...
{
    qDebug() << "Setting simulation time step to "
             << newTimeStep.value();
...
void Simulator::setOutputFolderLocation(
    QString newOutputFolderLocation)
{
    qDebug() << "Setting output directory to "
             << newOutputFolderLocation;
...
void Simulator::setExportInstantaneousTrajectory(
    bool exportInstaTraject, QString newInstaTrajectFilename)
{

    qDebug() << "Setting enable instantaneous file generation to "
             << exportInstaTraject << " with output file name to "
    ...
    if (!newInstaTrajectFilename.trimmed().isEmpty())
    {
        ...
        if (!fileInfo.completeSuffix().isEmpty())
        {
            ...
            this->mTrajectoryFilename = newInstaTrajectFilename;
        }
        else
        {
...
void Simulator::setExportIndividualizedShipsSummary(
...
void Simulator::initializeSimulation(bool emitSignal)
{
    ...
    connect(this, &Simulator::simulationFinished, this, [this]() {
        generateSummaryData();
                              ...
                              bool emitEndStepSignal)
{
           ...
           && (this->mSimulationTime <= this->mSimulationEndTime))
    {
        {
            QMutexLocker locker(
                &mutex); // Lock the mutex automatically

            // This will block the thread if pauseFlag is true
            while (mIsSimulatorPaused)
            { // Use a loop to re-check
              // the condition
                qWarning()
                    << "Simulation has been paused externally.";
        ...
        {
            // Emit the signal when all ships have reached their
            // destination
...
{
    return std::all_of(
        mShips.begin(), mShips.end(),
        [](const auto &s) { return !s->isShipStillMoving(); });
}
...
void Simulator::generateSummaryData()
{
    ...
    stream
        ...
        << "Simulation Time: " << Utils::formatDuration(difTime)
        << " (dd:hh:mm:ss)\n"
        << "~.~.~.~.~.~.~.~.~.~.~.~.~.~.~.~.~.~.~.~.~.~.~.~.~.~\n\n"
        << "+ NETWORK STATISTICS:\n"
        << "  |_ Total Number of Ships on Network                    "
           "                       \x1D : "
        << Utils::thousandSeparator(this->mShips.size()) << "\n"
        << "....................................................\n\n"
        << "\n";
    stream
        << "+ AGGREGATED/ACCUMULATED SHIPS STATISTICS:\n"
        << "    |-> Moved Commodity:\n"
        << "        |_ Total Moved Cargo (ton)                       "
           "                       \x1D : "
        << Utils::thousandSeparator(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips, std::function<double(
                               const std::shared_ptr<Ship> &)>(
                               [](const auto &s) {
                                   return s->getCargoWeight().value();
                               })))
        << "\n"
        << "        |_ Total ton.km (ton.Km)                         "
           "                       \x1D : "
        << Utils::thousandSeparator(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips,
                   std::function<double(
                       const std::shared_ptr<Ship> &)>(
                       [](const auto &s) {
                           return s->getTotalCargoTonKm().value();
                       })))
        << "\n\n"
        << "  |-> Route Information:\n"
        << "    |_ Ships Reached Destination                         "
           "                       \x1D : "
        << ShipNetSimCore::Utils::accumulateShipValuesInt(
               mShips,
               std::function<int(const std::shared_ptr<Ship> &)>(
                   [](const auto &s) {
                       return s->isReachedDestination();
                   }))
        << "\n"
        << "    |_ Ships Total Path Length (km)                      "
           "                       \x1D : "
        << Utils::thousandSeparator(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips,
                   std::function<double(
                       const std::shared_ptr<Ship> &)>(
                       [](const auto &s) {
                           return s->getTotalPathLength().value();
                       })))
        << "\n\n"
        << "  |-> Ships Performance:\n"
        << "    |_ Operating Time (d:h::m::s)                        "
           "                       \x1D : "
        << Utils::formatDuration(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips, std::function<double(
                               const std::shared_ptr<Ship> &)>(
                               [](const auto &s) {
                                   return s->getTripTime().value();
                               })))
        << "\n"
        << "    |_ Average Speed (meter/second)                      "
           "                       \x1D : "
        << ShipNetSimCore::Utils::accumulateShipValuesDouble(
               mShips,
               std::function<double(const std::shared_ptr<Ship> &)>(
                   [](const auto &s) {
                       return s->getTripRunningAvergageSpeed()
                           .value();
                   }))
               / this->mShips.size()
        << "\n"
        << "    |_ Average Acceleration (meter/square second)        "
           "                       \x1D : "
        << Utils::thousandSeparator(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips,
                   std::function<double(
                       ...
                       [](const auto &s) {
                           return s
                               ->getTripRunningAverageAcceleration()
                               .value();
                       }))
                   / this->mShips.size(),
               4)
        << "\n"
        << "    |_ Average Travelled Distance (km)                   "
           "                       \x1D : "
        << Utils::thousandSeparator(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips,
                   std::function<double(
                       const std::shared_ptr<Ship> &)>(
                       [](const auto &s) {
                           return s->getTraveledDistance().value()
                                  / 1000.0;
                       }))
               / this->mShips.size())
        << "\n"
        << "    |_ Consumed and Regenerated Energy:\n"
        << "        |_ Total Net Energy Consumed (KW.h)              "
           "                       \x1D : "
        << Utils::thousandSeparator(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips,
                   std::function<double(
                       const std::shared_ptr<Ship> &)>(
                       [](const auto &s) {
                           return s->getCumConsumedEnergy().value();
                       })))
        << "\n"
        << "            |_ Total Energy Consumed (KW.h)              "
           "                       \x1D : "
        << Utils::thousandSeparator(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips,
        ...
        << "\n"
        << "            |_ Total Energy Regenerated (KW.h)           "
           "                       \x1D : "
        << Utils::thousandSeparator(0.0) << "\n"
        << "            |_ Average Net Energy Consumption per Net "
           "Weight (KW.h/ton)         \x1D : "
        << Utils::thousandSeparator(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips,
                   std::function<double(const std::shared_ptr<Ship>
                                            &)>([](const auto &s) {
                       return s->getEnergyConsumptionPerTon().value();
                   })))
        << "\n"
        << "            |_ Average Net Energy Consumption per Net "
           "ton.km (KW.hx10^3/ton.km) \x1D : "
        << Utils::thousandSeparator(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips,
                   std::function<double(
                       const std::shared_ptr<Ship> &)>(
                       [](const auto &s) {
                           return s->getEnergyConsumptionPerTonKM()
                                      .value()
                                  * 1000.0;
                       })))
        << "\n"
    ...
    stream << "            |_ Total Overall Fuel Consumed (liters)   "
              "                          \x1D : "
           << Utils::thousandSeparator(
                  ShipNetSimCore::Utils::accumulateShipValuesDouble(
                      mShips,
                      std::function<double(
                          const std::shared_ptr<Ship> &)>(
                          [](const auto &s) {
                              return s->getOverallCumFuelConsumption()
                                  .value();
                          })))
           << "\n";
    ...
    for (auto &fc : totalFuelConsumed)
    {
        stream << Utils::formatString(
            "                |_ ",
            ShipFuel::convertFuelTypeToString(fc.first)
                + " (liters) ",
            "\x1D : ", " ", 84)
               << Utils::thousandSeparator(fc.second.value()) << "\n";
    }

    stream
        << "            |_ Average Fuel Consumed per Net Weight "
           "(litersx10^3/ton)           \x1D : "
        << Utils::thousandSeparator(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips,
                   std::function<double(
                       ...
                       [](const auto &s) {
                           return s
                               ->getOverallCumFuelConsumptionPerTon()
                               .value();
                       })))
        << "\n"
        << "            |_ Average Fuel Consumed per Net ton.km "
           "(litersx10^3/ton.km)       \x1D : "
        << Utils::thousandSeparator(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips,
                   std::function<double(const std::shared_ptr<Ship>
                                            &)>([](const auto &s) {
                       return s
                           ->getOverallCumFuelConsumptionPerTonKM()
                           .value();
                   })))
        << "\n"
        << "    |_ Environmental Impact:\n"
        << "        |_ Total CO2 Emissions (kg)                      "
           "                       \x1D : "
        << Utils::thousandSeparator(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips,
                   std::function<double(
                       const std::shared_ptr<Ship> &)>(
                       [](const auto &s) {
                           return s->getTotalCO2Emissions().value();
                       })))
        << "\n"
        << "        |_ Average CO2 Emissions per Net Weight (kg/ton) "
           "                       \x1D : "
        << Utils::thousandSeparator(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips,
                   std::function<double(
                       const std::shared_ptr<Ship> &)>(
                       [](const auto &s) {
                           return s->getTotalCO2EmissionsPerTon();
                       })))
        << "\n"
        << "        |_ Average CO2 Emissions per Net ton.km "
           "(kg/ton.km)                     \x1D : "
        << Utils::thousandSeparator(
               ShipNetSimCore::Utils::accumulateShipValuesDouble(
                   mShips,
                   std::function<double(
                       const std::shared_ptr<Ship> &)>(
                       [](const auto &s) {
                           return s->getCO2EmissionsPerTonKM();
                       })))
        << "\n"
        << "....................................................\n\n";
    ...
    for (auto &ship : mShips)
    {
        stream
            ...
            << "        |_ Total Moved Cargo (ton)                   "
               "                           \x1D : "
            << Utils::thousandSeparator(
                   ship->getCargoWeight().value())
            << "\n"
            << "        |_ Total ton.km (ton.km)                     "
               "                           \x1D : "
            << Utils::thousandSeparator(
                   ship->getTotalCargoTonKm().value())
            << "\n\n"
            << "  |-> Route Information:\n"
            << "    |_ Ships Reached Destination                     "
               "                           \x1D : "
            << (ship->isReachedDestination() ? "Yes" : "No") << "\n"
            << "    |_ Ships Total Path Length (km)                  "
               "                           \x1D : "
            << Utils::thousandSeparator(
                   ship->getTotalPathLength().value())
            << "\n\n"
            << "  |-> Ships Performance:\n"
            << "    |_ Operating Time (d:h::m::s)                    "
               "                           \x1D : "
            << Utils::formatDuration(ship->getTripTime().value())
            << "\n"
            << "    |_ Average Speed (meter/second)                  "
               "                           \x1D : "
            << ship->getTripRunningAvergageSpeed().value() << "\n"
            << "    |_ Average Acceleration (meter/square second)    "
               "                           \x1D : "
            << Utils::thousandSeparator(
                   ship->getTripRunningAverageAcceleration().value(),
                   4)
            << "\n"
            << "    |_ Average Travelled Distance (km)               "
               "                           \x1D : "
            << Utils::thousandSeparator(
                   ship->getTraveledDistance().value() / 1000.0)
            << "\n\n"
            << "    |_ Consumed and Regenerated Energy:\n"
            << "        |_ Total Net Energy Consumed (KW.h)          "
               "                           \x1D : "
            << Utils::thousandSeparator(
                   ship->getCumConsumedEnergy().value())
            << "\n"
            << "            |_ Total Energy Consumed (KW.h)          "
               "                           \x1D : "
            << Utils::thousandSeparator(
            ...
            << "            |_ Total Energy Regenerated (KW.h)       "
               "                           \x1D : "
            << Utils::thousandSeparator(0.0) << "\n"
            << "            |_ Average Net Energy Consumption per "
               "Net Weight (KW.h/ton)         \x1D : "
            << Utils::thousandSeparator(
                   ship->getEnergyConsumptionPerTon().value())
            << "\n"
            << "            |_ Average Net Energy Consumption per "
               "Net ton.km (KW.hx10^3/ton.km) \x1D : "
            << Utils::thousandSeparator(
                   ship->getEnergyConsumptionPerTonKM().value()
                   * 1000.0)
            << "\n\n"
            << "        |_ Tank Consumption:\n";
        ...
        stream << "            |_ Total Overall Fuel Consumed "
                  "(liters)                            \x1D : "
               << Utils::thousandSeparator(
                      ship->getOverallCumFuelConsumption().value())
               << "\n";
        ...
        stream
            << "            |_ Average Fuel Consumed per Net Weight "
               "(litersx10^3/ton)           \x1D : "
            << Utils::thousandSeparator(
                   ship->getOverallCumFuelConsumptionPerTon().value())
            << "\n"
            << "            |_ Average Fuel Consumed per Net ton.km "
               "(litersx10^3/ton.km)        \x1D : "
            << Utils::thousandSeparator(
                   ship->getOverallCumFuelConsumptionPerTonKM()
                       .value())
            << "\n\n"
            << "    |_ Environmental Impact:\n"
            << "        |_ Total CO2 Emissions (kg)                  "
               "                           \x1D : "
            << Utils::thousandSeparator(
                   ship->getTotalCO2Emissions().value())
            << "\n"
            << "        |_ Average CO2 Emissions per Net Weight "
               "(kg/ton)                        \x1D : "
            << Utils::thousandSeparator(
                   ship->getTotalCO2EmissionsPerTon())
            << "\n"
            << "        |_ Average CO2 Emissions per Net ton.km "
               "(kg/ton.km)                     \x1D : "
            << Utils::thousandSeparator(
                   ship->getCO2EmissionsPerTonKM())
            << "\n"
            << "...................................................."
               "\n\n";
    }
    ...
    shipsSummaryData =
        Utils::splitStringStream(mSummaryTextData, "\x1D :");

    ShipsResults rs = ShipsResults(
        shipsSummaryData, mTrajectoryFullPath, mSummaryFullPath);
...
void Simulator::exportSummaryToTXTFile()
{
    ...
    this->mSummaryFile.writeFile(
        mSummaryTextData.replace("\x1D", ""));
    // Close the file
...
void Simulator::runOneTimeStep()
{
    ...
    // Lock the mutex only for the duration of accessing or modifying
    // the mShips list
    {
        QMutexLocker locker(&mutex);
    ...
    {
        if (s->isReachedDestination() || !s->isShipStillMoving())
        {
            continue;
        }
    ...
    if (mPlotFrequency > 0.0
        && ((int(this->mSimulationTime) * 10) % (mPlotFrequency * 10))
               == 0)
    {
        ...
        for (std::shared_ptr<Ship> &s : (shipsToSimulate))
        {
            ...
            auto   data = std::make_pair(s->getUserID(), nP);
...
void Simulator::playShipOneTimeStep(std::shared_ptr<Ship> ship)
{
        ...
        && !ship->isLoaded())
    {
        ...
        for (std::shared_ptr<Ship> &otherShip : this->mShips)
        {
            ...
            if (otherShip == ship || !otherShip->isLoaded()
                || otherShip->isReachedDestination())
            {
                continue;
            }
        ...
        // skip loading the ship if there is any ship at the same
        // point
        if (!skipLoadingship)
        {
            // load ship
        ...
        && ship->isLoaded())
    {
        ...
        AlgebraicVector::Environment currentEnvironment =
            mNetwork->getEnvironmentFromPosition(
        ...
        if ((!ship->isCurrentlyDwelling())
            ...
            && ((std::round(ship->getPreviousSpeed().value() * 1000.0)
                 / 1000.0)
                == 0.0)
            && ((std::round(ship->getSpeed().value() * 1000.0)
                 / 1000.0)
                == 0.0)
        ...
        auto currentMaxSpeed = units::velocity::meters_per_second_t(
            100.0); // ship->getCurrentMaxSpeed();

        ship->sail(mSimulationTime, mTimeStep, currentMaxSpeed,
                   cp.gapToCriticalPoint, stopPoint.point,
                   cp.isFollowingAnotherShip, cp.speedAtCriticalPoint,
                   currentEnvironment);
        ...
        if (mExportTrajectory)
        {
            ...
            stream << mSimulationTime.value() << ","
                   ...
                   << QString::number(
                          ship->getTraveledDistance().value(), 'f', 3)
                   << ","
                   << QString::number(ship->getAcceleration().value(),
                                      'f', 3)
                   << ","
                   << QString::number(
                          ship->getSpeed()
                   ...
                   << ","
                   << QString::number(
                          ship->getCumConsumedEnergy().value(), 'f',
                          3)
                   << ","
                   << QString::number(
                          ship->getMainTankCurrentCapacity(), 'f', 3)
                   << ","
                   << ship->getCurrentPosition().toString("(%x; %y)")
                   << ","
                   << QString::number(
                          ship->getCurrentHeading().value(), 'f', 3);
...
units::time::second_t Simulator::getNotLoadedShipsMinStartTime()
{
    ...
    if (st.empty())
    {
        return units::time::second_t(-1.0);
    }
                            ...
                            int bar_length, bool emitProgressSignal)
{
    ...
    if (progressPercent != this->mProgressStep)
    {
        ...
        // Print the progress
        out << "Progress: [" << bar << "] " << progressPercent << "%"
            << ending;
...
{
    QMutexLocker locker(
        &mutex); // Use QMutexLocker to prevent potential deadlocks
