 ...
 * SimulatorAPI provides a thread-safe, event-driven interface for
 * maritime simulation with support for multiple networks, ships, and
 * simulation modes.
 *
 * Key Features:
 ...
 * - Continuous Mode: Continuous simulation with pause/resume
 * capabilities
 *
 ...
 * auto* network =
 * SimulatorAPI::InteractiveMode::loadNetwork("network.json",
 *                                                            "myNetwork");
 ...
 * @brief Custom locale facet to remove thousand separators from
 * numeric output
 *
 * @details This class customizes number formatting by removing
 * thousand separators (e.g., converts "1,000" to "1000"). Used
 * internally for consistent numeric string representation across
 * different locales.
 *
...
signals:
     ...
     * @param networkName The name of the network for which simulation
     * was created
     */
    void simulationCreated(QString networkName);

    /**
     * @brief Emitted when a network is successfully loaded into the
     * simulator.
    ...
    /**
     * @brief Emitted when simulations are paused (Continuous Mode
     * only).
     * @param networkNames List of network names whose simulations
     * were paused
     * @note In Async mode, signal is emitted only when all specified
     * networks are paused
     */
    void simulationsPaused(QVector<QString> networkNames);

    /**
     * @brief Emitted when paused simulations are resumed (Continuous
     * Mode only).
     * @param networkNames List of network names whose simulations
     * were resumed
     * @note In Async mode, signal is emitted only when all specified
     * networks are resumed
     */
    void simulationsResumed(QVector<QString> networkNames);

    /**
     * @brief Emitted when simulations are restarted from their
     * initial state.
     * @param networkNames List of network names whose simulations
     * were restarted
     * @note In Async mode, signal is emitted only when all specified
     * networks are restarted
     */
    ...
    /**
     * @brief Emitted when simulations complete their execution
     * naturally.
     * @param networkName network name whose simulation finished
     * @note Differs from simulationsTerminated which is for forced
     * termination
     */
     ...
     * @param currentSimulorTimePairs Map of network names to their
     * current time and progress The QPair contains <current
     * simulation time, progress percentage>
     */
    void simulationAdvanced(
     ...
     * @param currentSimulationProgress Map of network names to their
     * progress percentage (0-100)
     */
    void simulationProgressUpdated(QString networkName,
    ...
    /**
     * @brief Emitted when new ships are successfully added to a
     * simulation.
    ...
    /**
     * @brief Emitted when all ships in the simulation have reached
     * their destination
     ...
     * @param results Map of network names to their respective
     * simulation results
     */
    void
    simulationResultsAvailable(QPair<QString, ShipsResults> results);

    /**
     * @brief Emitted when simulation's current state information is
     * available.
     * @param simulatorState JSON object containing the simulator's
     * current state
     */
    void
    simulationCurrentStateAvailable(const QJsonObject simulatorState);

    /**
     * @brief Emitted when a ship's position is updated during
     * simulation.
     * @param shipID The unique identifier of the ship
     * @param position Current geographical position of the ship
     * @param heading Current heading angle of the ship
     * @param paths Vector of line segments representing the ship's
     * planned path
     */
    void shipCoordinatesUpdated(
        QString shipID, ShipNetSimCore::GPoint position,
        units::angle::degree_t                          heading,
        QVector<std::shared_ptr<ShipNetSimCore::GLine>> paths);

    /**
     * @brief Emitted when information about available ports is
     * requested.
     * @param NetworkPortIDs Map of network names to their available
     * port IDs
     */
    void
    availablePorts(QMap<QString, QVector<QString>> NetworkPortIDs);

#ifdef BUILD_SERVER_ENABLED

    /**
     * @brief Emitted when containers are successfully added to a
     * ship.
     * @param networkName The name of the network containing the ship
     * @param shipID The unique identifier of the ship to which
     * containers were added
     * @note This signal is only available when BUILD_SERVER_ENABLED
     * is defined
    ...
    /**
     * @brief Emitted when a ship carrying containers reaches a
     * seaport.
     * @param networkName The name of the network
     * @param shipID The unique identifier of the ship
     * @param seaPortCode The LOCODE of the seaport reached
     * @param containersCount Number of containers queued for leaving
     * the ship.
     * @note This signal is only available when BUILD_SERVER_ENABLED
     * is defined
     * @note The containers JSON array contains the state and details
     * of all containers at arrival
     */
     ...
     * @param shipID The unique identifier of the ship
     * @param seaPortCode The LOCODE of the seaport reached
     * @param containers JSON array containing information about the
     ...
     * @note This signal provides error information for error handling
     * and debugging
    ...
    /**
     * @brief Emitted when all worker threads complete their assigned
     * tasks.
     * @param networkNames List of networks whose workers have
     * completed
     * @note In Async mode, this signal is emitted only when all
     * workers are done
     * @note In Sync mode, this signal is emitted for each worker
     * completion
     * @see Mode
     */
...
protected:
    ...
    RequestData<QPair<units::time::second_t, double>>
        mTimeStepTracker;
     ...
     * @note Thread-safe implementation ensures single instance across
     * threads
     */
     ...
     * @param isExternallyControlled Whether external control is
     * enabled
    ...
    void createNewSimulationEnvironment(
        ...
        bool isExternallyControlled = false, Mode mode = Mode::Async);
     ...
     * @param networkName Identifier for the network (use "*" for
     * default)
     ...
     * @note Results are provided via simulationResultsAvailable
     * signal
     */
    void
     ...
     */
    void requestRestartSimulations(QVector<QString> networkNames);
    ...
    getShipByID(QString networkName, QString &shipID);
     ...
     */
    void requestShipCurrentStateByID(QString networkName, QString ID);
     ...
     * @note: portNames is a list of possible names that the port may
     * have.
     */
     ...
     */
    void addContainersToShip(QString networkName, QString shipID,
                             QJsonObject json);
#endif
     ...
     * @details Establishes all necessary connections between
     * simulator, ships, and API components
     */
     ...
     */
    bool
    checkAndEmitSignal(const int &counter, const int total,
    ...
    QVector<std::shared_ptr<ShipNetSimCore::Ship>>
    mLoadShips(QJsonObject &ships, QString networkName = "");
    QVector<std::shared_ptr<ShipNetSimCore::Ship>>
    ...
    mLoadShips(QVector<QMap<QString, std::any>> ships,
               QString                          networkName = "");
    QVector<std::shared_ptr<ShipNetSimCore::Ship>>
    mLoadShips(QString shipsFilePath, QString networkName = "");
...
protected slots:
     ...
     *
     * @details In Async mode, accumulates results until all tracked
     * ships reach destination. In Sync mode, emits signal
     * immediately. Emits shipsReachedDestination signal with
     * accumulated results.
     */
    ...
    void handleOneTimeStepCompleted(
        QString               networkName,
        units::time::second_t currentSimulatorTime, double progress,
        Mode mode);
     ...
     * @param endSimulationAfterRun If true, the simulation will end
     * after the current run
     * @param getStepEndSignal If true, a signal will be emitted at
     * the end of each step
     *
     * @details This method starts or continues the simulation for the
     * specified networks.
     * It supports both finite and infinite time steps, depending on
     * the value of `timeSteps`. If `endSimulationAfterRun` is true,
     * the simulation will terminate after the current run. If
     * `getStepEndSignal` is true, a signal will be emitted at the end
     ...
     * @param networkNames List of networks to pause the simulation
     * for
     *
     * @details This method pauses the simulation for the specified
     * networks. The simulation state is preserved and can be resumed
     * later using `resumeSimulation`.
     */
    void requestPauseSimulation(QVector<QString> networkNames);

    /**
     * @brief Resumes the simulation for the specified networks
     * @param networkNames List of networks to resume the simulation
     * for
     *
     * @details This method resumes the simulation for the specified
     * networks from the last paused state. The simulation continues
     ...
     * simulation for
     *
     * @details This method forcefully terminates the simulation for
     * the specified networks. The simulation cannot be resumed after
     * termination.
     */
     ...
     *
     * @details This method is called when a simulation finishes
     * naturally. It updates the simulation completion tracker and
     * emits the `simulationFinished` signal.
     */
     ...
     *
     * @details This method is called when a worker thread completes
     * its assigned tasks. It updates the worker readiness tracker and
     * emits the `workersReady` signal when all worker threads are
     * ready.
     */
     ...
     */
    void handleProgressUpdate(QString networkName, int progress);
     ...
     * m_shipsReachedBuffer and emits as a single
     * shipsReachedDestination signal. Clears the buffer after
     * emission.
     */
     ...
     * @param mode Current operation mode (Async/Sync)
     *
     * @details Processes port availability data and emits
     * availablePorts signal based on mode:
    ...
    void handleAvailablePorts(QString          networkName,
                              QVector<QString> portIDs, Mode mode);
...
public:
    ...
    loadShips(QJsonObject &ships, QString networkName = "");

    /**
     * @brief Load ships from JSON object using a specific network
     * instance
     * @param ships JSON object containing ship configurations
     * @param network Pointer to an initialized OptimizedNetwork
     * instance
     * @return Vector of loaded ship objects
     *
     * @details Similar to the networkName version but uses a direct
     * network reference Useful when working with a specific network
     * instance
     *
     * @throws Emits errorOccurred signal if network is nullptr or
     * JSON is invalid
     ...
     * @param ships Vector of maps containing ship properties as
     * strings
     * @param networkName Name of the network to associate ships with
     * @return Vector of loaded ship objects
     *
     ...
     * @param ships Vector of maps containing ship properties with
     * varied types
     * @param networkName Name of the network to associate ships with
     * @return Vector of loaded ship objects
     *
     ...
     * @return Vector of loaded ship objects
     *
     * @details The configuration file should be in JSON format
     * following the same structure as described in the JSON-based
     * loadShips method
     *
     ...
     * @note This method is thread-safe and uses the worker thread for
     * file operations
     */
    static QVector<std::shared_ptr<ShipNetSimCore::Ship>>
    loadShips(QString shipsFilePath, QString networkName = "");

    /**
     * @class SimulatorAPI::InteractiveMode
     * @brief Provides step-by-step control over maritime simulations
     *
     * @details The InteractiveMode class provides a static interface
     * for running simulations in a step-by-step manner, allowing
     * fine-grained control over simulation execution. This mode is
     * ideal for:
    ...
    public:
        ...
        /**
         * @brief Create a new simulation environment with explicit
         * network file
         * @param networkFilePath Path to the network configuration
         * file
         * @param networkName Unique identifier for the network
         * @param shipList Optional list of ships to initialize with
         * the simulation
         * @param timeStep Simulation time step duration (default: 1.0
         * second)
         * @param isExternallyControlled Whether simulation is
         * controlled externally
         * @param mode Synchronization mode for signal emission
         * @throws Emits errorOccurred if network file cannot be
         * loaded or name conflicts
         */
        ...
        /**
         * @brief Create a new simulation environment using existing
         * network
         ...
         * @param timeSteps Duration to run (use negative value for
         * infinite)
         */
         ...
         */
        static void finalizeSimulation(QVector<QString> networkNames);
         ...
         * @return Pointer to simulator or nullptr if not found
         */
        static ShipNetSimCore::Simulator *
        getSimulator(QString networkName);
         ...
         */
        static void
        requestAvailablePorts(QVector<QString> networkNames,
                              bool getOnlyPortsOnShipsPaths);
         ...
         * @param shipID  ship identifier
         * @param portNames List of names the port have.
         * @note: portNames is a list of possible names that the port
         * may have.
         */
        static void
     ...
     * @brief Provides continuous maritime simulation control with
     * pause/resume capabilities
     *
     * @details The ContinuousMode class offers a static interface for
     * running simulations continuously until completion or manual
     * intervention. This mode is ideal for:
    ...
    public:
         ...
         * file
         * @param networkName Unique identifier for the network
         * @param shipList List of ships to initialize with the
         * simulation
         * @param timeStep Simulation time step duration (default: 1.0
         * second)
         * @param isExternallyControlled Whether simulation is
         ...
         * @throws Emits errorOccurred if network file cannot be
         * loaded or name conflicts
         * @note Unlike InteractiveMode, shipList is required in
         * ContinuousMode
         */
        static void createNewSimulationEnvironment(
            QString networkFilePath, QString networkName,
         ...
         * @note Simulation will run until completion or manual
         * intervention
