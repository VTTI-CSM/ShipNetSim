...
void SimulatorAPI::setLocale()
{
    ...
    qtLocale.setNumberOptions(
        QLocale::OmitGroupSeparator); // Disable thousands separator
...
void SimulatorAPI::registerQMeta()
{
    ...
    qRegisterMetaType<ShipNetSimCore::GPoint>(
        "ShipNetSimCore::GPoint");
    qRegisterMetaType<
    ...
    qRegisterMetaType<units::length::meter_t>(
        "units::length::meter_t");
    qRegisterMetaType<units::angle::degree_t>(
    ...
    qRegisterMetaType<ShipNetSimCore::SeaPort>(
        "ShipNetSimCore::SeaPort");
}
...
void SimulatorAPI::resetInstance()
{
    ...
    if (instance)
    {
        ...
        for (const QString &networkName : networkNames)
        {
            ...
            {
                if (data.shipLoaderWorker->thread()
                    == data.workerThread)
                {
                    QMetaObject::invokeMethod(
...
SimulatorAPI::loadNetwork(QString filePath, QString networkName)
{
    ...
    if (filePath.trimmed().toLower() != "default")
    {
        ...
        {
            emit errorOccurred("Network file does not exist: "
                               + filePath);
            return nullptr;
        }
    }
    ...
    // Connect the network thread's started signal to initialize the
    // network
    connect(networkThread, &QThread::started, net,
            [this, networkName, filePath]() {
                ...
                if (filePath.trimmed().toLower() == "default")
                {
                    ...
                    auto filePaths = ShipNetSimCore::NetworkDefaults::
                ...
                APIData apiData = apiDataMap.get(networkName);
                apiData.network->initializeNetwork(fileLoc,
                                                   networkName);
            });
    ...
    connect(net, &ShipNetSimCore::OptimizedNetwork::errorOccurred,
            &loop, [&](QString error) {
                errorMsg = error;
...
APIData
SimulatorAPI::getApiDataAndEnsureThread(const QString &networkName)
{
    ...
    {
        throw std::runtime_error(
            QString("Worker thread for %1 is null!")
                .arg(networkName)
                .toStdString());
    }
    ...
    {
        throw std::runtime_error(
            QString("shipLoaderWorker for %1 is null!")
                .arg(networkName)
                .toStdString());
    }

    // Update the APIData in the thread-safe map (if any changes were
    // made)
    apiDataMap.addOrUpdate(networkName, apiData);
    ...
    QString networkName, Mode mode)
{
    ...
    {
        connect(
            ship.get(), &ShipNetSimCore::Ship::reachedDestination,
            this,
            [this, networkName, ship,
             mode](const QJsonObject shipState) {
                handleShipReachedDestination(networkName, shipState,
                                             mode);
            },
            mConnectionType);

        connect(
            ship.get(), &ShipNetSimCore::Ship::positionUpdated, this,
            [this,
             ship](ShipNetSimCore::GPoint currentPos,
                ...
                emit shipCoordinatesUpdated(
                    ship->getUserID(), currentPos, heading, path);
            },
            mConnectionType);
        ...
        connect(ship.get(), &ShipNetSimCore::Ship::reachedSeaPort,
                this,
                [this, ship, networkName](QString   shipID,
                                          QString   seaPortCode,
                                          qsizetype containersCount) {
                    emit SimulatorAPI::shipReachedSeaPort(
                        networkName, shipID, seaPortCode,
                        containersCount);
                });

        connect(ship.get(), &ShipNetSimCore::Ship::containersUnloaded,
                this,
                [this, ship, networkName](QString    shipID,
                                          QString    seaPortCode,
        ...
        connect(ship.get(), &ShipNetSimCore::Ship::shipStateAvailable,
                this, [this, ship, networkName](QJsonObject state) {
                    emit SimulatorAPI::shipStateAvailable(
                        networkName, ship->getUserID(), state);
                });
    }
}
...
SimulatorAPI::mLoadShips(QJsonObject &ships, QString networkName)
{
    ...
    {
        emit errorOccurred(
            "Invalid ship configuration: missing 'ships' array");
    ...
    try
    {
        ...
        QMetaObject::Connection connSuccess =
            connect(apiData.shipLoaderWorker,
                    &ShipLoaderWorker::shipsLoaded, this,
                    [&](QVector<std::shared_ptr<ShipNetSimCore::Ship>>
                            shipsVec) {
        ...
        QMetaObject::Connection connError = connect(
            apiData.shipLoaderWorker, &ShipLoaderWorker::errorOccured,
            this, [&](const QString &errMsg) {
                emit errorOccurred(
                    "Error loading ships in mLoadShips:" + errMsg);
                qWarning()
                    << "Error loading ships in mLoadShips:" << errMsg;
        ...
        QMetaObject::invokeMethod(
            ...
            [&, networkName]() {
                ...
                apiData.shipLoaderWorker->loadShips(
                    apiData.network, ships, networkName);
            },
            Qt::QueuedConnection);
        ...
        // 6) Once we exit the loop, disconnect our temporary
        // connections
                         ...
                         ShipNetSimCore::OptimizedNetwork *network)
{
    ...
    //    retrieve the existing APIData for the network
    APIData apiData =
        getApiDataAndEnsureThread(network->getRegionName());
    ...
    // 3) Ensure the network is in the same thread as the worker so
    // that
    //    the connections work properlly.
    if (network->thread() != apiData.workerThread)
    {
        QString error =
            QString("Error: Network is not in the expected worker "
                    "thread! "
                    "Expected thread: %1, Actual thread: %2")
                .arg(reinterpret_cast<quintptr>(apiData.workerThread),
                     0, 16)
                .arg(reinterpret_cast<quintptr>(network->thread()), 0,
                     16);
    ...
    try
    {
        ...
        //    synchronously from this function. So we'll use a local
        //    QEventLoop to block until we get a signal.
        // --------------------------------------------------------------------
        QEventLoop loop;
        ...
        QMetaObject::Connection connSuccess =
            connect(apiData.shipLoaderWorker,
                    ...
                    [&](QVector<std::shared_ptr<ShipNetSimCore::Ship>>
                            shipsVec) {
                        // Store the result in our local variable and
                        // quit the loop
                        loadedShips = shipsVec;
                        loop.quit();
                    });
    ...
    try
    {
        ...
        QMetaObject::Connection connError = connect(
            ...
            this, [&](const QString &errMsg) {
                ...
                emit errorOccurred(error);
                qWarning() << error;
                // We'll still quit the loop; 'loadedShips' stays
                // empty
                loop.quit();
            });
    ...
    try
    {
        ...
        QMetaObject::Connection connError = connect(
            ...
            this, [&](const QString &errMsg) {
                ...
                emit errorOccurred(error);
                qWarning() << error;
                // We'll still quit the loop; 'loadedShips' remains
                // empty
                loop.quit();
            });
        ...
        QMetaObject::invokeMethod(
            ...
            [&, networkName]() {
                // This lambda runs in the worker thread
                apiData.shipLoaderWorker->loadShips(
                    apiData.network, shipsFilePath, networkName);
            },
            Qt::QueuedConnection);
    ...
    Mode mode)
{
    ...
    // 2. Create SimulatorWorker INSIDE the worker thread using
    // invokeMethod()
    ...
    connect(apiData.simulatorWorker, &SimulatorWorker::errorOccurred,
            this,
            [this](QString error) { emit errorOccurred(error); });

    // 3. Setup the simulator safely inside the worker thread
    QMetaObject::invokeMethod(
        apiData.simulatorWorker,
        [&apiData, &shipList, timeStep, isExternallyControlled, this,
         networkName]() {
            apiData.simulatorWorker->setupSimulator(
    ...
    connect(
        ...
        [this, networkName]() { handleWorkersReady(networkName); },
        mConnectionType);
...
void SimulatorAPI::setupConnections(const QString &networkName,
                                    Mode           mode)
{
    ...
    connect(simulator,
            &ShipNetSimCore::Simulator::allShipsReachedDestination,
    ...
    connect(
        simulator, &ShipNetSimCore::Simulator::simulationFinished,
        this,
        [this, networkName]() {
    ...
    connect(
        ...
        this,
        [this, networkName,
         mode](units::time::second_t currentSimulatorTime,
    ...
    connect(
        simulator, &ShipNetSimCore::Simulator::progressUpdated, this,
    ...
    connect(
        simulator, &ShipNetSimCore::Simulator::simulationPaused, this,
    ...
    connect(
        simulator, &ShipNetSimCore::Simulator::simulationResumed,
        this,
        [this, networkName, mode]() {
    ...
    connect(
        simulator, &ShipNetSimCore::Simulator::simulationRestarted,
        this,
        [this, networkName, mode]() {
    ...
    connect(simulator, &ShipNetSimCore::Simulator::availablePorts,
            this, [this, networkName, mode](QVector<QString> ports) {
...
void SimulatorAPI::handleAllShipsReachedDestination(
    QString networkName)
{
                                        ...
                                        Mode             mode)
{
    ...
    case Mode::Async: {
        ...
        const int completed =
            mAvailablePortTracker.incrementAndGetCompleted();
    ...
    double progress, Mode mode)
{
    ...
    case Mode::Async: {
        ...
        if (completed == total)
        {
            ...
            QMap<QString, QPair<units::time::second_t, double>>
                timeData = mTimeStepTracker.getDataBuffer();
    ...
    default:
        emit errorOccurred(
            "Unexpected mode in handleOneTimeStepCompleted");
...
SimulatorAPI::getSimulator(QString networkName)
{
    // Check if the network exists in the thread-safe map
    if (!apiDataMap.contains(networkName))
    {
...
ShipNetSimCore::OptimizedNetwork *
SimulatorAPI::getNetwork(QString networkName)
{
    // Check if the network exists in the thread-safe map
    if (!apiDataMap.contains(networkName))
    {
...
void SimulatorAPI::requestRestartSimulations(
    QVector<QString> networkNames)
{
    // If "*" is specified, process all networks
    if (networkNames.contains("*"))
    ...
    for (const auto &networkName : networkNames)
    {
        ...
        if (apiData.simulator)
        {
            ...
            bool success = QMetaObject::invokeMethod(
                apiData.simulator, "restartSimulation",
                mConnectionType);
            if (!success)
            {
                emit errorOccurred(
                    "Failed to invoke restartSimulation");
    ...
    QVector<std::shared_ptr<ShipNetSimCore::Ship>> ships)
{
    ...
    for (auto &ship : ships)
    {
        ...
        bool success = QMetaObject::invokeMethod(
            apiData.simulator, "addShipToSimulation", mConnectionType,
            Q_ARG(std::shared_ptr<ShipNetSimCore::Ship>, ship));

        if (!success)
        {
            emit errorOccurred(
                "Failed to invoke addShipToSimulation");
...
std::shared_ptr<ShipNetSimCore::Ship>
SimulatorAPI::getShipByID(QString networkName, QString &shipID)
{
    // Check if the network exists in the thread-safe map
    if (!apiDataMap.contains(networkName))
    {
...
QJsonObject
SimulatorAPI::requestSimulatorCurrentState(QString networkName)
{
    // Check if the network exists in the thread-safe map
    if (!apiDataMap.contains(networkName))
    {
    ...
    if (!apiData.simulator)
    {
        emit errorOccurred("Simulator for network " + networkName
                           + " is not initialized!");
        return QJsonObject();
    }
                                       ...
                                       QJsonObject json)
{
    ...
    {
        emit errorOccurred("Ship with ID: " + shipID
                           + " does not exist");
    }
}
#endif
...
void SimulatorAPI::requestRunSimulation(
    QVector<QString> networkNames, units::time::second_t timeSteps,
    bool endSimulationAfterRun, bool getStepEndSignal)
{
    ...
    for (const auto &networkName : networkNames)
    {
        ...
        if (apiData.simulator)
        {
            ...
            bool success = QMetaObject::invokeMethod(
                apiData.simulator, "runSimulation", mConnectionType,
...
void SimulatorAPI::requestAvailablePorts(
    QVector<QString> networkNames, bool getOnlyPortsOnShipsPaths)
{
    ...
    for (const auto &networkName : networkNames)
    {
        ...
        if (apiData.simulator)
        {
            ...
            bool success = QMetaObject::invokeMethod(
                apiData.simulator, "getAvailablePorts",
                mConnectionType,
    ...
    QString networkName, QString shipID, QVector<QString> portNames)
{
    ...
    if (apiData.ships.contains(shipID))
    {
        ...
        apiData.ships.value(shipID)->requestUnloadContainersAtPort(
            portNames);
        return; // Return once the request is processed.
    }

    // Emit an error if the ship does not exist
    emit errorOccurred("A ship with ID " + shipID
                       + " does not exist!");
}
...
std::any SimulatorAPI::convertQVariantToAny(const QVariant &variant)
{
    ...
    case QMetaType::QVariantMap: {
        ...
        {
            anyMap[it.key().toStdString()] =
                convertQVariantToAny(it.value());
        }
...
void SimulatorAPI::emitShipsReachedDestination()
{
    if (!mReachedDesTracker.isDataBufferEmpty())
    {
        ...
        for (auto &network : networks)
        {
            ...
            concatenatedObject.insert(
                network, mReachedDesTracker.getData(network));
        }
...
bool SimulatorAPI::checkAndEmitSignal(
    ...
    void (SimulatorAPI::*signal)(QVector<QString>), Mode mode)
{
    ...
    Mode mode)
{
    ...
    getInstance().createNewSimulationEnvironment(
        networkFilePath, networkName, shipList, timeStep,
        isExternallyControlled, mode);
}
    ...
    Mode mode)
{
    mMode = mode;
    getInstance().createNewSimulationEnvironment(
        networkName, shipList, timeStep, isExternallyControlled,
        mode);
}

ShipNetSimCore::OptimizedNetwork *
SimulatorAPI::InteractiveMode::loadNetwork(QString filePath,
                                           QString networkName)
{
...
std::shared_ptr<ShipNetSimCore::Ship>
SimulatorAPI::InteractiveMode::getShipByID(QString  networkName,
                                           QString &shipID)
{
...
ShipNetSimCore::OptimizedNetwork *
SimulatorAPI::ContinuousMode::loadNetwork(QString filePath,
                                          QString networkName)
{
    return getInstance().loadNetwork(filePath, networkName);
}
...
{
    units::time::second_t timeSteps = units::time::second_t(
        std::numeric_limits<double>::infinity());
...
{
    return SimulatorAPI::getInstance().mLoadShips(shipsFilePath,
                                                  networkName);
}
