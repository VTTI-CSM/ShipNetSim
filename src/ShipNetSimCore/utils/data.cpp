...
bool Data::CSV::writeLine(const QString &line)
{
    ...
    mOutStream << line
               << Qt::endl; // Use Qt::endl to flush the stream
...
bool Data::CSV::clearFile()
{
    ...
    // Open the file in write-only mode without append to truncate its
    // content
    if (!mFile.open(QIODevice::WriteOnly | QIODevice::Text
                    | QIODevice::Truncate))
    {
...
Data::Table
Data::CSV::read(const bool hasHeaders, const QString &separator,
                ...
                int filterColumnIndex)
{
    ...
    const int LINES_PER_CHUNK =
        10'000; // low for high performance and memory

    QList<QFuture<QVector<QVector<QVariant>>>> futures;

    // Define a mutex if needed for shared data (currently not
    // required) QMutex mutex;

    // Define a lambda for processing lines
    auto processLines =
        [&](const QStringList &lines) -> QVector<QVector<QVariant>> {
        ...
        for (const QString &rawLine : lines)
        {
            ...
            QVector<QString> rowStr =
                line.split(separator, Qt::KeepEmptyParts);
    ...
    for (const QVector<QVariant> &row : allData)
    {
        for (int i = 0; i < table.headers.size(); ++i)
        {
            ...
            {
                table.tableMap[table.headers[i]].append(
                    QVariant("")); // Fill missing with empty QVariant
            }
        }
...
QVector<QString> Data::CSV::getDistinctValuesFromCSV(
    const bool hasHeaders, int columnIndex, const QString &separator)
{
    ...
    constexpr qint64 CHUNK_SIZE =
        10 * 1024 * 1024; // Process 10MB at a time
    QList<QFuture<QSet<QString>>>
           futures; // Store futures for parallel processing
    ...
    while (!in.atEnd())
    {
        ...
        // Lock file access to read a chunk
        {
            ...
            chunkData =
                file.read(CHUNK_SIZE); // Read a chunk of the file
        }
    ...
    return QVector<QString>(distinctValues.begin(),
                            distinctValues.end());
}
                            ...
                            const QString          &separator)
{
    ...
    while (!in.atEnd())
    {
        ...
        for (qsizetype i = 0; i < table.headers.size(); ++i)
        {
            ...
            {
                double value =
                    rowStr[i].toDouble(&conversionSuccessful);
                if (conversionSuccessful)
                {
                    table.tableMap[table.headers[i]].push_back(value);
                }
            ...
            {
                table.tableMap[table.getHeaders()[i]].push_back(
                    rowStr[i]);
            }
            else
            {
                                          ...
                                          QString &filename)
{
    ...
    // Create a QTextStream object to write the XML content to the
    // file
...
Data::ProjectFile::readProjectFile(const QString &filename)
{
    ...
    if (!doc.setContent(&file))
    {
        file.close();
        throw std::runtime_error(
            "Error: Failed to parse the XML file.");
    }
    ...
    QDomElement projectElement =
        root.firstChildElement("ProjectName");
    QDomElement networkElement =
        root.firstChildElement("NetworkName");
    QDomElement authorElement = root.firstChildElement("AuthorName");
    QDomElement shipsElement =
        root.firstChildElement("ShipsFileName");
    QDomElement simEndTimeElement =
        root.firstChildElement("simEndTime");
    QDomElement simTimestepElement =
        root.firstChildElement("simTimestep");
    QDomElement simPlotTimeElement =
        root.firstChildElement("simPlotTime");
