...
protected:
    ...
    // this is effecient since i will need to loop over the whole ds
    // anyways and we do not know the size of the table
...
public:
    ...
    template <typename T>
    inline T getCellData(const QString &headerName,
                         qsizetype      index) const
    {
        ...
        if (!cell.canConvert<T>())
        {
            // Or more specific:
            throw std::runtime_error(
                QString("Cannot convert cell data at header '%1' "
                        "index %2 "
     ...
     *                                          [](const Cell& cell)
     * -> bool { if (cell.type() == typeid(int)) { return
     * boost::get<int>(cell) > 30;
     *      }
     *      return false;
     *  });
     */
        ...
        std::function<bool(const Cell &)> filterFunction) const
    {
        ...
        for (qsizetype rowIdx = 0; rowIdx < column.size(); ++rowIdx)
        {
            ...
            {
                // If the row matches the filter, add all column
                // values of this row to the filtered table
                for (const auto &header : headers)
                {
    ...
    /**
     * @brief Get distinct values from a specified column with type
     * checking
     * @tparam T The type of values to retrieve
     * @param columnName Name of the column to get distinct values
     * from
     * @return QSet of distinct values of type T from the specified
     * column
     * @throws std::runtime_error if column name is not found or if
     * type mismatch occurs
     * @code
     *    QSet<int> uniqueAges = table.getDistinctValues<int>("age");
     *    QSet<QString> uniqueNames =
     * table.getDistinctValues<QString>("name");
     */
    template <typename T>
    inline QSet<T> getDistinctValues(const QString &columnName) const
    {
        ...
        // Use QSet constructor to efficiently create the set in one
        // step
    ...
    template <typename T>
    inline static Table
    createFromColumns(const QVector<QString>    &headers,
                      const QVector<QVector<T>> &data)
    {
        ...
        {
            throw std::runtime_error(
                "Number of headers must match number "
                "of columns in data");
        }
        ...
        for (int colIdx = 0; colIdx < headers.size(); ++colIdx)
        {
            ...
            {
                tableColumn.push_back(
                    QVariant::fromValue(data[colIdx][rowIdx]));
            }
        }
    ...
    template <typename T>
    inline static Table
    createFromRows(const QVector<QString>    &headers,
                   const QVector<QVector<T>> &data)
    {
        // Validate input
        if (headers.isEmpty())
        ...
        const QVector<QPair<firstColumnType, secondColumnType>> &data)
    {
        ...
        {
            // Add `pair.first` to the column corresponding to
            // `headers.first`
            ...
            {
                throw std::runtime_error(
                    "firstColumnType must "
                    "be convertible to QVariant");
            }

            // Add `pair.second` to the column corresponding to
            // `headers.second`
            if constexpr (std::is_convertible<secondColumnType,
                                              QVariant>::value)
            {
    ...
    public:
        using inner_const_iterator =
            QMap<QString, QVector<Cell>>::const_iterator;

        const_iterator(inner_const_iterator it)
            : it_(it)
        {
        }
...
public:
    ...
    Table
    read(const bool                           hasHeaders = false,
         const QString                       &separator  = ",",
         std::function<bool(const QString &)> filterFunc = nullptr,
         int                                  filterColumnIndex = -1);
    QVector<QString>
    getDistinctValuesFromCSV(const bool hasHeaders, int columnIndex,
                             const QString &separator);
...
SHIPNETSIM_EXPORT void createProjectFile(const ProjectDataFile pf,
                                         QString &filename);

SHIPNETSIM_EXPORT ProjectDataFile
readProjectFile(const QString &filename);
}; // namespace ProjectFile
} // namespace Data
}; // namespace ShipNetSimCore
#endif // DATA_H
