...
private:
    // Constants
    static const qint64 MAX_TRAJECTORY_SIZE =
        1024 * 1024; // 1 MB in bytes
    static const int COMPRESSION_LEVEL =
        9; // Highest compression level

    QVector<QPair<QString, QString>>
        summaryData;               // Stores the summary
                                   // data (key-value pairs)
    QByteArray trajectoryFileData; // Stores the content of the
                                   // trajectory file
    QString trajectoryFileName;    // Stores the full path of the
                                   // trajectory file
    QString
        summaryFileName; // Stores the full path of the summary file
...
public:
    ...
    ShipsResults(QVector<QPair<QString, QString>> summary,
        ...
        : summaryData(summary)
        , summaryFileName(summaryFilePath)
    {
    ...
    QVector<QPair<QString, QString>> getSummaryData()
    {
        return summaryData;
    }
    QByteArray getTrajectoryFileData()
    {
    ...
    // Function to save the trajectory file content to an optional new
    // path
    bool saveTrajectoryFile(const QString &newPath = QString()) const
    {
        QString savePath =
            newPath.isEmpty() ? trajectoryFileName : newPath;
    ...
    {
        QString savePath =
            newPath.isEmpty() ? summaryFileName : newPath;
    ...
    QJsonObject toJson() const
    {
        ...
        if (trajectoryFileData.size() <= MAX_TRAJECTORY_SIZE)
        {
            ...
            json["trajectoryFileData"] =
                QString(trajectoryFileData.toBase64());
    ...
    static ShipsResults fromJson(const QJsonObject &json)
    {
        ...
        for (const auto &value : summaryArray)
        {
            ...
            results.summaryData.append(
                qMakePair(key, pairObject[key].toString()));
        }
        ...
        if (json["trajectoryFileDataIncluded"].toBool(false))
        {
            QByteArray base64Data =
                json["trajectoryFileData"].toString().toUtf8();
            results.trajectoryFileData =
        ...
        results.trajectoryFileName =
            json["trajectoryFileName"].toString();
