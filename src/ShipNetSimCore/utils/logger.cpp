...
void Logger::attach(QString fileBaseName)
{
    ...
    if (!fileOpened)
    {
        qWarning() << "Logger could not open file at"
                   << Logger::logPath;
    }
}
                     ...
                     const QString            &msg)
{
    ...
    if (Logger::logging && type >= Logger::fileMinLogLevel)
    {
        ...
        QString txt =
            QString("%1 - %2: %3 %4 line: %5\r\n")
                .arg(QDateTime::currentDateTime().toString(), logType,
                     msg, context.file,
    ...
    // For terminal output, we bypass the default handler and
    // implement our own to ensure proper color handling
    ...
    {
        // For messages we don't want on console, still call default
        // handler (it might direct to other outputs)
...
// Sets the Min level of logging for the std output (as per Qt
// Framework)
