...
class HierarchicalVisibilityGraphTest : public QObject
{
    Q_OBJECT
...
private slots:
    ...
    void testVisibilityWithinPolygon();
    void testVisibilityBetweenPolygons();
    ...
    void testFindShortestPathAlternate();
...
private:
    ...
    std::shared_ptr<HierarchicalVisibilityGraph>   visibilityGraph;
    QVector<std::shared_ptr<GPoint>>               testPoints;
};

void HierarchicalVisibilityGraphTest::initTestCase()
{
    qDebug() << "Starting HierarchicalVisibilityGraph tests...";

    // Create test polygons (water bodies)
    auto polygons = createTestPolygons();

    // Create visibility graph
    visibilityGraph = std::make_shared<HierarchicalVisibilityGraph>(
        polygons);
...
void HierarchicalVisibilityGraphTest::cleanupTestCase()
{
    qDebug() << "HierarchicalVisibilityGraph tests completed.";
}

QVector<std::shared_ptr<Polygon>>
HierarchicalVisibilityGraphTest::createTestPolygons()
{
...
HierarchicalVisibilityGraphTest::createTestSeaPorts()
{
...
void HierarchicalVisibilityGraphTest::testDefaultConstructor()
{
    HierarchicalVisibilityGraph defaultGraph;

    // Should be in a valid but empty state
    QVERIFY(true); // Constructor should not throw
}

void HierarchicalVisibilityGraphTest::testParameterizedConstructor()
{
...
void HierarchicalVisibilityGraphTest::testClear()
{
    // Create a copy for testing
    auto                          polygons = createTestPolygons();
    HierarchicalVisibilityGraph   testGraph(polygons);
...
void HierarchicalVisibilityGraphTest::testIsVisible()
{
...
void HierarchicalVisibilityGraphTest::testIsSegmentVisible()
{
    auto segment =
...
void HierarchicalVisibilityGraphTest::testVisibilityWithinPolygon()
{
    ...
    if (containingPolygon)
    {
        ...
        // Check visibility from test point to polygon vertices
        int visibleCount = 0;
        auto outerRing = containingPolygon->outer();
        for (int i = 0; i < outerRing.size() - 1 && i < 10; ++i)
        {
            bool vis = visibilityGraph->isVisible(
                testPoints[0], outerRing[i]);
            if (vis) visibleCount++;
            if (i < 5)
            {
                qDebug() << "Visible to vertex" << i << ":"
                         << vis << outerRing[i]->toString();
            }
        }

        qDebug() << "Visible vertices count:" << visibleCount;

        // Should see at least some polygon vertices
...
void HierarchicalVisibilityGraphTest::
    testVisibilityBetweenPolygons()
{
...
void HierarchicalVisibilityGraphTest::testAddManualVisibleLine()
{
    auto manualLine =
...
void HierarchicalVisibilityGraphTest::testClearManualLines()
{
...
void HierarchicalVisibilityGraphTest::testFindShortestPath()
{
    ...
    if (result.isValid())
    {
        qDebug() << "SUCCESS: Found valid path";
    ...
    {
        qDebug() << "ISSUE: No path found - this "
                    "indicates the bug!";
        ...
        auto startNav =
            visibilityGraph->getLevel0Quadtree()->findNearestNeighborPoint(
                startPoint);
        auto endNav =
            visibilityGraph->getLevel0Quadtree()->findNearestNeighborPoint(
                endPoint);
...
void HierarchicalVisibilityGraphTest::testFindShortestPathAlternate()
{
    qDebug() << "\n=== TESTING ALTERNATE SHORTEST PATH ===";
    ...
    auto result =
        visibilityGraph->findShortestPath(startPoint, endPoint);
    ...
    if (result.isValid())
    {
        ...
        qDebug() << QString("=== TOTAL PATH LENGTH: %1 meters ===")
                        .arg(totalLength, 0, 'f', 2);
    ...
    {
        qDebug() << "ISSUE: No path found - this indicates "
                    "the bug!";
    ...
    testFindShortestPathMultiplePoints()
{
    ...
    auto result = visibilityGraph->findShortestPath(waypoints);
    ...
    else
    {
        ...
        {
            auto segmentResult =
                visibilityGraph->findShortestPath(
...
void HierarchicalVisibilityGraphTest::testFindContainingPolygon()
{
...
void HierarchicalVisibilityGraphTest::testMapBoundaries()
{
    auto minPoint = visibilityGraph->getMinMapPoint();
    auto maxPoint = visibilityGraph->getMaxMapPoint();
...
void HierarchicalVisibilityGraphTest::testConnectWrapAroundPoints()
{
...
void HierarchicalVisibilityGraphTest::testVisibilityDebugging()
{
...
void HierarchicalVisibilityGraphTest::testPathFindingDebugging()
{
    ...
    if (startPolygon && endPolygon)
    {
        ...
        // Try path finding
        auto simpleResult = visibilityGraph->findShortestPath(
    ...
    else
    {
            ...
            && !polygons.first()->outer().isEmpty())
        {
            ...
            auto vertexResult =
                visibilityGraph->findShortestPath(vertex1,
                                                  vertex2);
...
void HierarchicalVisibilityGraphTest::testEdgeCases()
{
    qDebug() << "\n=== EDGE CASES ===";

    // Test with same start and end point
    qDebug() << "--- Testing same point path ---";
    auto samePointResult = visibilityGraph->findShortestPath(
    ...
    auto outsideResult = visibilityGraph->findShortestPath(
...
void HierarchicalVisibilityGraphTest::debugIntersectionDetection()
{
...
void HierarchicalVisibilityGraphTest::debugGLineIntersection()
{
...
void HierarchicalVisibilityGraphTest::debugAStarInvalidPath()
{
    qDebug() << "\n=== DEBUGGING INVALID PATH ===";

    // The problematic segment from path
...
QTEST_MAIN(HierarchicalVisibilityGraphTest)
#include "test_hierarchicalvisibilitygraph.moc"
